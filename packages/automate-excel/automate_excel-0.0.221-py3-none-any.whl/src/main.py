...
from automate_excel import config
...
class Range(object):
    ...
    def comment(self, text: str):
        ...
        """
        ## TODO: allow for multiple comments on a single cell (check first that this is possible in Excel)
        self.clear('comments')
        if text is not None:
    ...
    def values(self, values):
        ...
        if isinstance(values, pd.core.frame.DataFrame):
            cols = tuple(col for col in values.columns)
