...
def rpc_handler(api_handler: object, server: Server, path: str = '/',
                method: str = 'POST', debug: bool = False):
    executor = iprpc.MethodExecutor(api_handler)
    server.add_route(method, path, partial(_request_handler, executor, debug))


async def _request_handler(executor: iprpc.MethodExecutor,
                           debug: bool,
                           ctx: Span,
                           request: web.Request) -> web.Response:
    data = await request.read()
    ctx.annotate(str(data))
    result = await executor.call(data, request.charset,
                                 const_args={'ctx': ctx})
