...
def mkapp(handler):
    ...
    async def home_handler(request: web.Request):
        try:
            ...
            result = await caller.call(req_bytes,
...
async def exec(aiohttp_server, handler, request: bytes,
               error: Optional[ErrType]):
...
async def test_error_deserialization(aiohttp_server):
    ...
    await exec(aiohttp_server, TestHandler,
               b'{1}',
               iprpc.DeserializeError)
...
async def test_sync(aiohttp_server):
    ...
    data = await exec(aiohttp_server, TestHandler,
...
async def test_internal_exec_cache(aiohttp_server):
    ...
    async with aiohttp.ClientSession() as sess:
        resp = await sess.post(
            server.make_url('/'),
            data=b'{"method": "hello", "params": {}}')
        data = await resp.json()
        assert data['result'] == 1

        resp = await sess.post(
            server.make_url('/'),
            data=b'{"method": "hello2", "params": {}}')
        data = await resp.json()
...
async def test_missing_arg1(aiohttp_server):
    ...
    res = await exec(aiohttp_server, TestHandler,
