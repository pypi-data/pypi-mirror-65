...
.. note::
   ...
   Per poter gestire i :ref:`tornei solitari <corona carrom>` ogni incontro ha due valori,
   normalmente nascosti, che riportano il numero complessivo di *tiri sbagliati* per ciascun
   giocatore, da cui viene derivato lo *score*.

   Quei valori vengono normalmente inseriti dai giocatori stessi, usando un modulo che devono
   compilare alla fine di ciascun turno: è chiaramente compito di chi gestisce il torneo
   controllare che i valori immessi corrispondano al vero. È possibile aggiornare la tabella
   dei risultati degli incontri usando l'azione :guilabel:`Aggiorna` disponibile nel menu che
   viene mostrato cliccando nella tabella col tasto destro del mouse.

   Quando tutti gli incontri risultano compilati, il gestore del torneo potrà procedere ad
   aggiornare la classifica e quindi a generare il turno successivo: ogni volta che viene
   generato un turno, SoL invia automaticamente una mail a ciascun giocatore che contiene l'URL
