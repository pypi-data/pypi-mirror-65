...
def training_match_form(request):
    ...
    if cnum == 1:
        ...
        contender = m.competitor2.player1
        score = m.score2
    else:
        player = m.competitor2.player1
        contender = m.competitor1.player1
        score = m.score1
...
def adjust_scores(s1, s2):
    """Reduce the given scores so that neither one exceeds 25.
...
def store_training_match(request):
    ...
    if not all(e and e.isdigit() and 0 < int(e) < 99 for e in errors):
        return {
            '_': gettext,
            'error': gettext(_('All boards must be entered,'
                               ' as integer numbers between 1 and 99!')),
            'ngettext': ngettext,
            'championship': m.tourney.championship,
            'tourney': m.tourney,
            'currentturn': m.tourney.currentturn,
    ...
    if cnum == 1:
        ...
        if m.misses2:
            m.score1, m.score2 = adjust_scores(m.misses2, m.misses1)
    else:
