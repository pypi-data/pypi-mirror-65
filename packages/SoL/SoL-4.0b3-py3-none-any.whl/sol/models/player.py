...
In the next round you are going to play against {contender}: following the
...
class Player(GloballyUnique, Base):
    ...
    def sendTrainingURL(self, request, match, cnum, contender):
        loc = request.registry.queryUtility(ILocalizer, name=self.language)
        if loc is None:
            tdirs = request.registry.queryUtility(ITranslationDirectories, default=[])
            loc = make_localizer(self.language, tdirs)
        t = loc.translate
        cname = t(contender.caption(html=False, ignore_privacy=True))
        ...
        body = t(TRAINING_URL_MESSAGE).format(firstname=self.firstname,
                                              ...
                                              contender=cname, nboards=nboards, url=url)
