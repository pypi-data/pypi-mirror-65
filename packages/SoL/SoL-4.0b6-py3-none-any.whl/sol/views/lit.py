...
def training_match_form(request):
    ...
    if cnum == 1:
        already_entered = m.misses1 is not None or m.score1
    else:
        already_entered = m.misses2 is not None or m.score2
...
def store_training_match(request):
    ...
    if not all(e and e.isdigit() and 0 <= int(e) < 100 for e in errors):
        return {
            '_': gettext,
    ...
    changes_logger.info('%r self updated his errors for %r: %s', player, m, ', '.join(errors))

    tot_errors = sum(int(e) for e in errors)
    if cnum == 1:
        m.misses1 = tot_errors
    else:
        m.misses2 = tot_errors

    if m.misses1 is not None and m.misses2 is not None:
        tboards = m.tourney.championship.trainingboards
        m.score1, m.score2 = errors_to_scores(m.misses1, m.misses2, tboards)
