...
ENABLE_DATA_ACTIONS = argument.Argument("enable_data_actions", "--enable-data-actions", "", action='store_true',
                                        default=None, help="Enable data action enforcement on workspace",
                                        required=False)

DISABLE_DATA_ACTIONS = argument.Argument("disable_data_actions", "--disable-data-actions", "", action='store_true',
                                         default=None, help="Disable data action enforcement on workspace",
                                         required=False)
...
def update_workspace(
        ...
        enable_data_actions=None,
        disable_data_actions=None,
        logger=None):

    # Returns a dict containing the update details.
    if enable_data_actions and disable_data_actions:
        raise UserErrorException(
            'Arguments {} and {} cannot be specified at the same time'.format(ENABLE_DATA_ACTIONS.long_form,
                                                                              DISABLE_DATA_ACTIONS.long_form))

    if enable_data_actions:
        module_logger.warning("Note, this will enable data plane action enforcement on this workspace. "
                              "As a result existing role assignments with only control plane actions "
                              "(including the default Owner and Contributor role) will start denying "
                              "any data plane actions for the users. Please read more here - "
                              "https://docs.microsoft.com/en-us/azure/machine-learning/how-to-assign-roles")

    if disable_data_actions:
        module_logger.warning("Note, this will disable data plane action enforcement on this workspace. "
                              "As a result existing role assignments which were denying data plane actions "
                              "will stop getting enforced and result in possible enhanced privileges for the users. "
                              "Please read more here - "
                              "https://docs.microsoft.com/en-us/azure/machine-learning/how-to-assign-roles")

    if disable_data_actions:
        enable_data_actions = False

    return workspace.update(friendly_name=friendly_name, description=description, tags=tags,
                            image_build_compute=image_build_compute, enable_data_actions=enable_data_actions)
