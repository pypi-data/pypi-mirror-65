...
class ParallelRunStep(_PythonScriptStepBase):
    ...
    def _get_pystep_inputs(self):
        ...
        def _convert_to_dataref(index, in_ds):
            constructed_df = []
            if isinstance(in_ds, PipelineOutputFileDataset):
                # Copy the PipelineData so that the name of the PipelineDataOutputFileDataset is what the user chose.
                pipeline_data_copy = copy.copy(in_ds._pipeline_data)
                pipeline_data_copy._name += '_0'
                constructed_df.append(pipeline_data_copy)
            elif isinstance(in_ds, DatasetConsumptionConfig):
