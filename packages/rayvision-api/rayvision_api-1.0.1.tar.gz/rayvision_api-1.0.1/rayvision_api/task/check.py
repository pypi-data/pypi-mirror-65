...
class RayvisionCheck(object):
    ...
    def execute(self, task_info, upload_info):
        ...
        task_info = self.check_task_info(task_info)
        upload_info = upload_info or {}

        self.check_error_warn_info()
        self.is_scene_have_error()  # Check error.
        scene_info_render = task_info.get("scene_info_render") or task_info["scene_info"]
        self._edit_param_and_write(scene_info_render, task_info["task_info"], upload_info)
        self.logger.info('[Rayvision_utils check end .....]')
        return task_info["task_info"]["task_id"]
