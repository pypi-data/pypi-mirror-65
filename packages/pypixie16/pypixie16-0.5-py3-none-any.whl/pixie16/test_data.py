...
# test files
fname0 = "2019-08-16-test-alpha-det0018_mod00.bin"

if fname0[-3:] != "bin":
    print("Need a PIXIE-16 binary file")
    sys.exit()
...
trailsum = np.array(trailsum)
gapsum = np.array(gapsum)
leadsum = np.array(leadsum)
...
def own_CFD(w, num_traces):
    ...
    for i in range(num_traces):
        plt.figure()
        plt.plot(t, trace[i], label="Trace")
        plt.plot(tcfd, CFD[i], label="CFD")
        plt.plot(tff, FF[i], label="Fast Filter")
        plt.plot([t[0], t[-1]], [CFDth, CFDth], label="CFD Threshold")
        plt.plot([t[0], t[-1]], [FFth, FFth], label="FF Threshold")
        plt.plot(cfdtime[i], 0, ".", ms=10)
        plt.legend()
        plt.show()


def energy_filter(num_traces, calc_own_energy=False):
    ...
    for i in range(num_traces):
        ...
        plt.figure()
        plt.plot(trace[i], label="Trace")
        plt.plot(FF[i], label="Fast filter")
        plt.plot(SF[i], label="Slow filter")
        plt.plot(
            [0, trace.shape[1]],
        ...
        plt.plot(
            [EsampleFF, EsampleFF], [FF[i].min(), FF[i].max()], "k--", label="E, FF"
        )
        plt.plot(
            [Esample[i], Esample[i]],
            [SF[i].min(), SF[i].max()],
            "b--",
            label="E sample, E sums",
        )
        plt.legend()
