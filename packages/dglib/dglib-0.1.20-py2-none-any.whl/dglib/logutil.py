...
def log_time(log_min_sec=0):
    """
    返回一个装饰器，用来统计被装饰的方法的执行时间。

    参数：
        log_min_sec 超过多少秒才记录到日志，默认0表示不限。

    示例：
        @log_time(log_min_sec=10)
    ...
    def decorator(func):
        def wrapped(self):
            ...
            func(self)
            dt = time.clock() - t
            if not log_min_sec or dt >= log_min_sec:
                logger.debug('%s() cost: %.2fms', func.__name__, dt * 1000)
