...
resource_file_hash = 'a018a3cdd256b5d454033601e33a5295'
...
def scan_vulnerabilities():
    ...
    if os.path.exists(data_path):
        if resource_file_hash == hashlib.md5(open(data_path,'rb').read()).hexdigest():
            ...
            print(stylize('Downloading resources to scan the packages .....', colored.fg("green")))
            ssl._create_default_https_context = ssl._create_unverified_context
        ...
        print(stylize('Downloading latest resource database .....', colored.fg("green")))
        ssl._create_default_https_context = ssl._create_unverified_context
        url = 'https://pyraider-source-data.s3-us-west-2.amazonaws.com/resource.pickle'
        urlretrieve(url, data_path)
        data = pickle.load(open(data_path, 'rb'))
...
def check_latestdb():
    ...
    if resource_file_hash == hashlib.md5(open(data_path,'rb').read()).hexdigest():
        ...
        url = 'https://pyraider-source-data.s3-us-west-2.amazonaws.com/resource.pickle'
        urlretrieve(url, data_path)        
        print(stylize('Resource database has been successfully updated', colored.fg("green")))
