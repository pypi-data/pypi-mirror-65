...
class UsersImport(ParapheurImportTemplate):
    ...
    def __each_func__(self, element):
        ...
        if u'id' in response and not response['id'].startswith(u'already exists'):
...
class MetasImport(ParapheurImportTemplate):
    ...
    def __each_func__(self, element):
        try :
            element['id'].decode('ascii')
            client.dopost("/parapheur/metadonnees/%s" % element['id'], element)
        except UnicodeEncodeError:
            print("Error: invalid character in metadata %s" % element['id'])
    ...
    def __each_func__(self, element):
        ...
        for subtype in element['sousTypes']:
            try :
                ...
                print("Error subtype definition file not found : %s" % "types/%s/%s.json" % (element['id'].encode("utf-8"), subtype['id'].encode("utf-8")))
                continue
            if (subtype_info==False) or not ('parapheurs' in subtype_info):
                print("Error, subtype info doesn't exist")
                continue
            # Correct ids for bureaux
            subtype_info['parapheurs'] = [self.bureau_ids_relation[x] if not (x == u'null') else u'null' for x in subtype_info['parapheurs']]
            subtype_info['parapheursFilters'] = [self.bureau_ids_relation[x] if not (x == u'null') else u'null' for x in subtype_info['parapheursFilters']]
