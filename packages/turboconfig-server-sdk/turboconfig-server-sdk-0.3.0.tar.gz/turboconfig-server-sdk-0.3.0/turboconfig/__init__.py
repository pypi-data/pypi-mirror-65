...
from .util import tc_logger, _PY_
...
def initialize(sdk_key):
    if _PY_ == 2 and isinstance(sdk_key, basestring):  # noqa F821
        _set_sdk_key(sdk_key)
    elif isinstance(sdk_key, str):
        _set_sdk_key(sdk_key)
    else:
        raise Exception("Invalid SDK key found")
    return get_instance()
...
__all__ = ["get_instance", "initialize", "Identity", "TCClient"]
