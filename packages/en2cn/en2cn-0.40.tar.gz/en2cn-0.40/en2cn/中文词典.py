...
import os
#0. 导入词典相关常量
from .翻译 import 数据类型 as 模型
from .翻译 import 词典相关常量 as 词典常量
from .翻译 import  自定义词典 as 自定义词典 
#from . import 词典数据 as 词典数据
...
def 导入词典数据():
    ...
    while i < 16 :
        print(i)
        #open(os.path.join(os.path.dirname(__file__), 'template', 'tree_template.html')) 
        fr1 = open(os.path.join(os.path.dirname(__file__), '词典数据/词典'+ str(i) +'.ts'),'r+',encoding='utf-8')
    ...
    # 2. 导入 词形变化数据 
    fr1 = open(os.path.join(os.path.dirname(__file__), "词典数据/词形变化.ts"),'r+',encoding='utf-8')
    dic2 = eval(fr1.read())   #读取的str转换为字典
    #print(dic["Asama"])
    #print(dic["-ability"])
    fr1.close()
    #词形变化数据 = dic2
