    ...
    def update(self, instance, validated_data):
        ...
        if (new_email is not None) and (new_email != user.email):
            ...
            try:
                ...
                if EmailAddress.objects.filter(email__iexact=new_email).first() is not None:
                    logger.info('User "{}" wants to reset email "{}" as primary'.format(requesting_user, new_email))
            ...
            if not restore_as_primary:
                ...
                set_session_key(request, '__django_sso_app__user_email_updated', True)
...
class NewUserSerializer(UserSerializer):
    ...
    def create(self, validated_data):
        ...
        new_user = create_local_user_from_object(validated_data, not must_confirm_email)

        # setting new_user profile serializer field
        setattr(new_user, 'profile', update_profile(new_user.sso_app_profile, validated_data.get('profile')))
        ...
        try:
            new_user_subscriptions =  request.data.get('profile', {}).get('subscriptions', [])
            if len(new_user_subscriptions) > 0:
                for new_subscription in new_user_subscriptions:
