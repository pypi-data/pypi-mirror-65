...
_urlpatterns = [
    url(r'^$', UserApiViewSet.as_view({'get': 'list', 'post': 'create'}), name='list'),

    url(r'^(?P<sso_id>[-_\w]+)/$',
        UserApiViewSet.as_view({'get': 'retrieve', 'patch': 'partial_update', 'delete': 'delete_test_user'},
                               name='detail'),
        name='detail'),
]
...
extra_urlpatterns = (format_suffix_patterns([
                         ...
                         url(r'^get-revisions/$', UserRevisionsApiView.as_view(), name="get_users_revisions")]),
