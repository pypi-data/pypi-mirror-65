...
CLASS_PATH_ERROR = 'votebase is unable to interpret settings value for %s. '\
...
def get_model_string(model_name):
    ...
    """
    setting_name = 'VOTEBASE_%s_MODEL' % model_name.upper().replace('_', '')
    class_path = getattr(settings, setting_name, None)

    if not class_path:
        return 'votebase.%s' % model_name
