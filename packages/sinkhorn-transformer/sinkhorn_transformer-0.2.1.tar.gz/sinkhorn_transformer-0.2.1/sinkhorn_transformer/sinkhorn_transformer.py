...
class CausalAttentionSortNet(nn.Module):
    def __init__(self, heads, buckets, dim, dim_sort):
        ...
        self.linear_sort_k = nn.Parameter(torch.randn(1, heads, 2 * dim, dim_sort))
    ...
    def forward(self, q, k):
        ...
        Wsk = expand_dim(self.linear_sort_k, 0, b).reshape(bh, 2 * dim, dim_sort)
...
class SinkhornCausalAttention(nn.Module):
    def __init__(self, buckets, dim, heads, dropout = 0., kv_buckets = None, attn_sort_net = False):
        ...
        if attn_sort_net:
            self.sort_net = CausalAttentionSortNet(heads, buckets, dim_heads, dim_heads // 2)
        else:
