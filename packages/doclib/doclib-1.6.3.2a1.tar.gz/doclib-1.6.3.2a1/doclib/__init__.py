...
class Bot:
    ...
    async def start(self, function = None, advanced = False):
    ...
    def initiate_pm(self, id, sender_id = None, bot = None):
        if sender_id == None:
            ...
            for msgJSON in self.conn:
                i += 1
                msg = AttrDict(json.loads(msgJSON))
                if msg.type == "pm-reply":
                    to = msg
                    if bot != None:
                        bot.copy_to(to.pm_id)
                    else:
                        self.copy_to(to.pm_id)
                    return to
                if i > self.API_timeout:
                    botlog("pm-reply API response not recorded. reason: too many events before pm-reply.", error = True)
                    return None
        else:
            return "unimplemented"

    async def copy_to(self, roomName, password = None):
        ...
        copy.set_regexes(self.regexes)
        copy.connect()
        self.copies.append(copy)
        botTask = asyncio.create_task(copy.start())
        await botTask
