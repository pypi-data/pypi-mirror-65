...
class Bot:
    ...
    def log(self, message, error = False):
        print((ERROR if error else NORMAL) + message)
        sys.stdout.flush()

    def __init__(self, nick, room = "bots", owner = None, password = "", help = None, ping = None, important = False, killed = None, API_timeout = 10, advanced = False, function = None):
        ...
        if isinstance(parent, AttrDict):
            ...
            for msgJSON in self.conn:
                ...
                if i > self.API_timeout:
                    self.log("send-reply API response not recorded. reason: too many events before send-reply.", True)
                    return None
            ...
            raise BadMessageError(f'message not sent, because type of parent was {type(parent)}').with_traceback(f'parent = {parent}', error = True)
    ...
    def start(self, function = None, advanced = False):
        if not (self.advanced or advanced):
            ...
            if self.function == None and function != None and callable(function):
                ...
                self.log("No function passed to advanced bot's start function.")
            try:
                for msgJSON in self.conn:
                    msg = AttrDict(json.loads(msgJSON))
    ...
    def _handle_auth(self, pw):
        ...
        for msgJSON in self.conn:
            ...
            if msg.type == "auth-reply":
                ...
                break
            if i > self.API_timeout:
                self.log("Auth Error: auth-reply API response not recorded.", error = True)
                break
    ...
    def get_userlist(self):
        ...
        for msgJSON in self.conn:
            ...
            if i > self.API_timeout:
                self.log("who-reply API response not recorded. reason: too many events before who-reply.", error = True)
                return None
...
class KilledError(BotError):
    def __init__(self, message = "bot killed."):
        ...
        self.log(f'KilledError: {message}', error = True)

class BadMessageError(BotError):
    def __init__(self, message):
        self.message = message
        self.log(f'BadMessageError: {message}', error = True)
