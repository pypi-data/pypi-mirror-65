...
api_instance = vrt_lss_routing.MatrixApi(vrt_lss_routing.ApiClient(configuration))
body = vrt_lss_routing.RoutingTask() # RoutingTask | Новый запрос на расчет матрицы

try:
    # Построение матрицы расстояний и времени
    api_response = api_instance.build_matrix(body)
    pprint(api_response)
