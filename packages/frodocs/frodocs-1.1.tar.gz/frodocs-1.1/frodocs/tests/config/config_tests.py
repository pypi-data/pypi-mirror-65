...
import frodocs
from frodocs import config
from frodocs.config import config_options
from frodocs.exceptions import ConfigurationError
from frodocs.tests.base import dedent
...
class ConfigTests(unittest.TestCase):
    ...
    def test_config_option(self):
        ...
        Allows users to specify a config other than the default `frodocs.yml`.
        """
        expected_result = {
            'site_name': 'Example',
            'pages': [
        ...
        with TemporaryDirectory() as temp_path:
            ...
            config_path = os.path.join(temp_path, 'frodocs.yml')
    ...
    def test_theme(self):
        with TemporaryDirectory() as mytheme, TemporaryDirectory() as custom:
            ...
            results = (
                {
                    'dirs': [os.path.join(theme_dir, 'frodocs'), frodocs_templates_dir],
                    ...
                    'dirs': [os.path.join(theme_dir, 'readthedocs'), frodocs_templates_dir],
                    'static_templates': ['404.html', 'sitemap.xml'],
                    'vars': {
                    ...
                    'dirs': [mytheme, frodocs_templates_dir],
                    'static_templates': ['sitemap.xml'],
                    'vars': {}
                }, {
                    'dirs': [custom, os.path.join(theme_dir, 'readthedocs'), frodocs_templates_dir],
                    'static_templates': ['404.html', 'sitemap.xml'],
                    'vars': {
                        'include_search_page': True,
                        'search_index_only': False,
            ...
            for config_contents, result in zip(configs, results):
                ...
                c = config.Config(schema=(('theme', config_options.Theme(default='frodocs')),))
    ...
    def test_empty_nav(self):
        ...
        conf.load_dict({
            'site_name': 'Example',
            'config_file_path': os.path.join(os.path.abspath('.'), 'frodocs.yml')
        })
        conf.validate()
    ...
    def test_doc_dir_in_site_dir(self):
        ...
        conf = {
            'config_file_path': j(os.path.abspath('..'), 'frodocs.yml')
        }
