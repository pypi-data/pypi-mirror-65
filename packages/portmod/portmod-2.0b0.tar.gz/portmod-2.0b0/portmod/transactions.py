...
from .repo.util import get_max_version, select_mod
...
def generate_transactions(
    ...
    *,
    ...
    @param noreplace: If true, don't re-install mods that are already installed
    @param update: If true and noreplace, re-install mods that have updates available
    @param newuse: If true and noreplace, re-install mods with use changes
    @param delete: If true, don't add mods that should be enabled, but aren't.
                   Transactions will only be created for mods that have been disabled
    ...
    if not delete:
        for atom in enabled:
            ...
            if noreplace and installed is not None:
                ...
                if update and (
                    version_gt((to_install or installed).ATOM.MVR, installed.ATOM.MVR)
                    or (installed.LIVE and full_load_mod(installed).can_update_live())
                ):
                ...
                if (
                    (
                        ...
                        and installed.MF == to_install.MF
                    )
                    or installed is not None
                    and to_install is None
                ):
                    if installed.ATOM.CMN in newselected:
                        transactions.append(Trans.REINSTALL, to_install or installed)
                elif installed is not None and to_install is not None:
                    ...
                    # We could only reach here if to_install is not None
