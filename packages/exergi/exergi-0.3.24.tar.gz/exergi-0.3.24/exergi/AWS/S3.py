...
def exportPandasToS3(obj,bucket,key):
    ...
    # JSON files
        ...
        if objClass == "keras":
            with tempfile.TemporaryFile(suffix='.json') as fp:
                # STILL NOT WORKING? 
                fp.write(json.dumps(obj.to_json()))    # Error message: a bytes-like object is required, not 'str'
                fp.seek(0)
                s3resource.Object(bucket, key).put(Body=fp.read())
    ...
    # HDF5 files 
        ...
        if objClass == "keras":
            with tempfile.TemporaryFile(suffix='.h5')  as fp:
                # STILL NOT WORKING? 
                obj.save_weights(fp.name)   # Error message: expected str, bytes or os.PathLike object, not int
                fp.seek(0)
                s3resource.Object(bucket, key).put(Body=fp.read())
    else:
        raise Exception(raiseString)
...
def importFileFromS3(bucket, key, objClass="pandas", **kwargs):
    ...
    # Pickle files 
        ...
        if objClass == "pandas":
            ...
            with tempfile.TemporaryFile() as fp:
                s3resource.download_fileobj(Bucket=bucket, Key=key,Fileobj=fp)
                fp.seek(0)
                obj = joblib.load(fp)          
        else:
            raise Exception(raiseString)
