...
import kuob.recorder as recorder
import kuob.asciicast.raw as raw
import kuob.asciicast.v2 as v2
import kuob.notifier as notifier
from kuob.api import APIError
from kuob.commands.command import Command
    ...
    def execute(self):
        ...
        if upload:
            ...
            try:
                ...
                self.print_error("retry later by running: kuob upload %s" % self.filename)
                return 1
        else:
            self.print_info("asciicast saved to %s" % self.filename)
