...
Idle time limit, as given via `-i` option to `kuob rec`. Float.
...
Command that was recorded, as given via `-c` option to `kuob rec`. String.

#### `title`

Title of the asciicast, as given via `-t` option to `kuob rec`. String.
...
> Official kuob recorder captures only `SHELL` and `TERM` by default. All
...
> Official kuob recorder doesn't capture stdin by default. All
> implementations of asciicast-compatible terminal recorder should not capture
...
* [kuob terminal recorder](https://github.com/kuob/kuob) - 2.0
  (to be released, currently on development branch)
* [kuob web player](https://github.com/kuob/kuob-player) - 2.6.0
* [kuob server](https://github.com/kuob/kuob-server) - v20171105
