...
from kuob import __version__
import kuob.config as config
from kuob.commands.auth import AuthCommand
from kuob.commands.record import RecordCommand
from kuob.commands.play import PlayCommand
from kuob.commands.cat import CatCommand
from kuob.commands.upload import UploadCommand
...
def main():
    if locale.nl_langinfo(locale.CODESET).upper() not in ['US-ASCII', 'UTF-8']:
        print("kuob needs an ASCII or UTF-8 character encoding to run. Check the output of `locale` command.")
        sys.exit(1)
  ...
  Record terminal and upload it to kuob.sh:
    \x1b[1mkuob rec\x1b[0m
  Record terminal to local file:
    \x1b[1mkuob rec demo.cast\x1b[0m
  Record terminal and upload it to kuob.sh, specifying title:
    \x1b[1mkuob rec -t "My git tutorial"\x1b[0m
  Record terminal to local file, limiting idle time to max 2.5 sec:
    \x1b[1mkuob rec -i 2.5 demo.cast\x1b[0m
  Replay terminal recording from local file:
    \x1b[1mkuob play demo.cast\x1b[0m
  Replay terminal recording hosted on kuob.sh:
    \x1b[1mkuob play https://kuob.sh/a/difqlgx86ym6emrmd8u62yqu8\x1b[0m
  Print full output of recorded session:
    \x1b[1mkuob cat demo.cast\x1b[0m

For help on a specific command run:
  \x1b[1mkuob <command> -h\x1b[0m""",
        formatter_class=argparse.RawDescriptionHelpFormatter
    )
    parser.add_argument('--version', action='version', version='kuob %s' % __version__)
    ...
    parser_upload = subparsers.add_parser('upload', help='Upload locally saved terminal session to kuob.sh')
    ...
    parser_auth = subparsers.add_parser('auth', help='Manage recordings on kuob.sh account')
