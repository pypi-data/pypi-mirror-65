...
class Config:
    ...
    def upgrade(self):
        ...
        if self.env.get('KUOB_API_TOKEN'):
            raise ConfigError('KUOB_API_TOKEN variable is no longer supported, please use KUOB_INSTALL_ID instead')
    ...
    @property
    def install_id(self):
        id = self.env.get('KUOB_INSTALL_ID') or self.__read_install_id()
...
def get_config_home(env=os.environ):
    env_kuob_config_home = env.get("KUOB_CONFIG_HOME")
    env_xdg_config_home = env.get("XDG_CONFIG_HOME")
    env_home = env.get("HOME")

    config_home = None

    if env_kuob_config_home:
        config_home = env_kuob_config_home
    elif env_xdg_config_home:
        config_home = path.join(env_xdg_config_home, "kuob")
    elif env_home:
        if path.isfile(path.join(env_home, ".kuob", "config")):
            # location for versions < 1.1
            config_home = path.join(env_home, ".kuob")
        else:
            config_home = path.join(env_home, ".config", "kuob")
    else:
        raise Exception("need $HOME or $XDG_CONFIG_HOME or $KUOB_CONFIG_HOME")
