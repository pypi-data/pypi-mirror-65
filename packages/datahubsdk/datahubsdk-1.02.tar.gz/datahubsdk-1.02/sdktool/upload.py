...
class Upload(object):
    ...
    def commit(self):
        ...
        if not (self.erp and self.projectpath and (self.dataunit or self.partition)):
            logging.error("Please make erp/projectpath/dataunit(partition) are assigned correctly!!!")            
            return  
    ...
    def put_single(self,item,resource,hashv="",fp=""):
        ...
        for project_path in self.projectpath:
            project_id = BasicConfig.path2id(project_path)
            meta_item = {"size":size,"filePath":fp,"hash":hashv,"cfspath":cfs_path,"project_id":project_id}
