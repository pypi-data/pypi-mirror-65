...
def combine_model_name_and_version(
        model_name: str,
        model_version: str,
        delimiter: str = "_",
...
def split_model_name_and_version(
        model_locator: str,
        model_name_pattern: Union[str, None] = DEFAULT_MODEL_NAME_PATTERN,
        delimiter_pattern: str = DEFAULT_DELIMITER_PATTERN,
        version_pattern: str = DEFAULT_VERSION_PATTERN
...
def find_highest_compatible_version(
    ...
    def _run_search(search_list, v_index, version_elements):
        ...
        for available_version in search_list:
            ...
            model_name, model_version = split_model_name_and_version(
                available_version,
...
def find_matching_model_names(
    ...
    """ Return the keys with model names that match the search_pattern.

        By default, this expects that the model keys follow the convention of
            {{model name}}_{{model version}}
        e.g.
            MyModel_0.1.0
            my_model_0.1.0
        ...
        Args:
            ...
            delimiter_pattern (r string): Regex pattern describing delimiter
                Not recommended to modify this pattern.
        Returns:
            List[str]: List of keys that match the search.
    ...
    matched_keys = []
    for key in available_model_locators:
        model_name, model_version = split_model_name_and_version(
            key, model_name_pattern, delimiter_pattern, version_pattern)
