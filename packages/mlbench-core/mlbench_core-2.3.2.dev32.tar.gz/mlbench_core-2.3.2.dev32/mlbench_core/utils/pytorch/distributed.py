...
class Aggregation(object):
    ...
    def _agg(self, batch_idx, data, op):
    ...
    def _agg_grad_by_model(self, batch_idx, model, op):
        ...
        logger.info("packed")
        aggregated = self._agg(batch_idx, packed, op=op)
...
class AllReduceAggregationFP16(AllReduceAggregation):
    def _agg(self, batch_idx, data, op):
        ...
        if op == "avg":
            batch = [batch_idx]
            batch = hvd.allreduce(batch, op=hvd.Sum)
            print(dist.get_rank(), batch)
            assert batch[0] == batch_idx *2
