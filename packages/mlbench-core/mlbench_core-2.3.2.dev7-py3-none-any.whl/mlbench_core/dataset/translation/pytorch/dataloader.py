...
class WMT14Dataset(Dataset):
    ...
    def __init__(
        ...
        """
        tmp =  "train" if train else "val"
        logger.info("building {}".format(tmp))
        self.lazy = lazy
        super(WMT14Dataset, self).__init__(examples=[], fields={})
        logger.info("built {}".format(tmp))
        if download:
            path = self.download(root)
        else:
            path = os.path.join(root, "wmt14")

        logger.info("Downloaded {}".format(tmp))
        ...
        logger.info("Built tokenizers for {}".format(tmp))
