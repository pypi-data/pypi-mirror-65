...
def global_average(sum, count):
    def helper(array):
        ...
        logger.info("Aggregating average for {}".format(array))
        dist.all_reduce(array, op=dist.reduce_op.SUM)
        logger.info("Average aggregated {}".format(array))
...
class Aggregation(object):
    ...
    def _agg_model_by_model(self, model, op):
        ...
        """
        # Pack all layers
        packed, indices, sizes = pack_tensors([t.data for t in model.parameters()], use_cuda=self.use_cuda)
    ...
    def _agg_grad_by_model(self, model, op):
        ...
        """
        # Pack all layers
        packed, indices, sizes = pack_tensors([t.grad.data for t in model.parameters()], use_cuda=self.use_cuda)
        aggregated = self._agg(packed, op=op)
    ...
    def _agg_grad_by_layer(self, model, op):
        """Aggregate models gradients.
    ...
    def agg_grad(self, by_layer=False):
        if by_layer:
            return self._agg_grad_by_layer
        else:
            return self._agg_grad_by_model
...
class AllReduceAggregation(Aggregation):
    ...
    def __init__(self, world_size, use_cuda=False):
        ...
        super(AllReduceAggregation, self).__init__(use_cuda)
...
class DecentralizedAggregation(Aggregation):
    ...
    def __init__(self, rank, neighbors, use_cuda=False):
        ...
        super(DecentralizedAggregation, self).__init__(use_cuda)
...
class SparsifiedAggregation(Aggregation):
    ...
    def __init__(self, model, use_cuda=False):
        super(SparsifiedAggregation, self).__init__(use_cuda)
        pass
