...
class CentralizedAdam(Adam):
    ...
    Args:
        ...
        use_horovod (bool): Whether to use horovod to broadcast tensors (when using FP16 tensors with MPI)
    """
    ...
    def __init__(
        ...
        self.model = model
        if use_horovod:
            self.agg = AllReduceAggregationFP16(world_size=world_size, use_cuda=use_cuda).agg_grad(by_layer=by_layer)
        else:
            self.agg = AllReduceAggregation(world_size=world_size, use_cuda=use_cuda).agg_grad(by_layer=by_layer)
