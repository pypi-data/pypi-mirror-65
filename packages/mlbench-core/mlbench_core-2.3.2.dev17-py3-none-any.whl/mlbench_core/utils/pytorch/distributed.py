...
def unpack_to_tensors(packed, indices, sizes):
    tensors = []
    start_indices = indices[:-1]
    end_indices = indices[1:]
    for i, (start, end) in enumerate(zip(start_indices, end_indices)):
        tensors.append(packed[start: end].view(
            sizes[i]
        ))
...
class Aggregation(object):
    ...
    def agg_model(self, model, op):
        ...
        # Unpack
        unpacked = unpack_to_tensors(aggregated, indices, sizes)
        for agg, param in zip(unpacked, model.parameters()):
            param.data = agg
