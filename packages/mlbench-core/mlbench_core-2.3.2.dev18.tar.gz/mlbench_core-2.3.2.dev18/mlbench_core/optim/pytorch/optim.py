...
class CentralizedSGD(SGD):
    ...
    def __init__(
        ...
        self.model = model
        self.agg = AllReduceAggregation(world_size=world_size, use_cuda=use_cuda).agg_grad
...
class CentralizedAdam(Adam):
    ...
    def __init__(
        ...
        self.model = model
        if use_horovod:
            self.agg = AllReduceAggregationFP16(world_size=world_size, use_cuda=use_cuda).agg_grad
        else:
            self.agg = AllReduceAggregation(world_size=world_size, use_cuda=use_cuda).agg_grad
