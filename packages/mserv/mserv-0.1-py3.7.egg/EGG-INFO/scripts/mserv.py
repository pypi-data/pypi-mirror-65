#!/home/quinton/.pyenv/versions/3.7.7/bin/python3
...
def run(max_ram: "Maximum amount of ram alloted" = "-Xmx1024M", min_ram: "Minimum amount of ram alloted" = "-Xms1024M",
    ...
    subprocess.run(["java", f"{max_ram}", f"{min_ram}", "-jar", f"{os.path.join(serverDir[serverName], 'server.jar')}", f"{gui}"],
                   cwd=serverDir[serverName])
