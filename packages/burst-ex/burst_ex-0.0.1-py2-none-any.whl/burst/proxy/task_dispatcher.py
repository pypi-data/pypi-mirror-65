...
class TaskDispatcher(object):
    ...
    def _try_replace_workers(self):
        ...
        for group_id, _workers in list(self.busy_workers_dict.items()):
            if _workers:
                # 还有在运行中的workers
                return False
        ...
        self.idle_workers_dict = dict(
            [(group_id, set() | _workers) for group_id, _workers in list(self.reload_helper.workers_dict.items())]
        )
        ...
        # 分配现有的idle workers
        for group_id, _workers in list(bk_idle_workers_dict.items()):
