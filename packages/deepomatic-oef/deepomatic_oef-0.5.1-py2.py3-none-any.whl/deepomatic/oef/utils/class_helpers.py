...
def get_module_and_class_from_protobuf_descriptor(message):
    ...
    assert field_type.startswith(package_name), 'Protobuf malformation: the protobuf file ({}) does not starts with its package ({})'.format(field_type, package_name)
    if not field_type.startswith(package_name):
        return None
