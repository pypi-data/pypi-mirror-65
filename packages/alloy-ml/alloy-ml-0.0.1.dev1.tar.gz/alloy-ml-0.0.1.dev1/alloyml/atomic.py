...
tab_filename = path.join(dirname, 'atomic_features.csv')
table = pd.read_csv(tab_filename, index_col=0)
properties = table.columns
elements = table.index


def prop_value(el, prop):
    return table.loc[el, prop]
...
class Mean:
    ...
    >>> mean_sos
    mean(Speed of sound)
    >>> alloys = pd.read_csv(path.join(dirname, '..', 'data', AAl_maxUTS_MIF.csvsv), index_col=0)
...
class StdDev:
    ...
    >>> std_sos
    std(Speed of sound)
    >>> alloys = pd.read_csv(path.join(dirname, '..', 'data', Al_maxUTS_MIF.csvcsv), index_col=0)
    >>> alloy = alloys.iloc[24]
    >>> std_sos(alloy)
    1272.5303415141127
    """
...
class FeatureMap:
    ...
    >>> alloys = pd.read_csv(path.join(dirname, '..', 'data',Al_maxUTS_MIF.csvsv'), index_col=0)
    >>> a = alloys.iloc[24]
    >>> a.name
    '2030-T3510'
    >>> b = feat_map(a)
