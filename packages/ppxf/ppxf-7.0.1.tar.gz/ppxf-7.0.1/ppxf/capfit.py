...
def lsqlin(A_lsq, b_lsq, A_ineq=None, b_ineq=None, A_eq=None, b_eq=None,
    ...
    if A_ineq is None:
        ...
        r = E @ u - f
        assert linalg.norm(r) > 0, "Constraints are incompatible"
        x = Vs @ (-r[:-1]/r[-1] + fu)
...
class capfit:
    ...
    def __init__(self, func, p1, abs_step=None, bounds=(-np.inf, np.inf),
                 diff_step=1e-4, fixed=None, ftol=1e-4, max_nfev=None,
        ...
        while(True):
            ...
            h = lsqlin(A, b, A_ineq, b_ineq_p, A_eq, b_eq_p, bounds=bounds-p1)
