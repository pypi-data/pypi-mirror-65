...
from rslib.algo.atrank import ATRankforRL
...
class controllerObs(object):
    def __init__(self, config, model, modelfile, sess, src, batch_size=None, one_step=False, use_rule=False):
        ...
        self.mid_layer_model = tf.keras.backend.function(self.model.input, self.model.get_layer('at_rank_layer').output)
    ...
    def keras_emb(self, rawstate):
        ...
        with session.as_default():
            with session.graph.as_default():
                ...
                mid_layer_output = np.reshape(mid_layer_output,[np.shape(mid_layer_output)[0], -1])
...
class L20RecEnv(gym.Env):
    ...
    def __init__(self, batch_size=None, one_step=False, reward_sqr=False, use_rslib_model=False, use_rule=True, reward_type='ctr'):
        ...
        modelfile = '/root/rslib/demo/mystic/code/output/0315atrank2.ckpt'
        obs_size = 1361
        if batch_size:
            self.batch_size = batch_size
            self.batch = True
        ...
        model, sess = ATRankforRL.get_model(config, return_session=True)
