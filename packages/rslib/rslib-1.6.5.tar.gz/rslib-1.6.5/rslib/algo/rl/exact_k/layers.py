...
def ptn_rnn_decoder(cell,
    ...
    with tf.variable_scope("decoder_rnn") as scope:
        ...
        def custom_sample_from_logits(logits):
            _, top4 = tf.nn.top_k(logits, 4)
            prob = tf.constant([[4, 3, 2, 1]],dtype=tf.float32)
        ...
        if (mode in ['SAMPLE', "GREEDY", "CUSTOM"]):
            if mode == "SAMPLE":
                ...
                if_taohua_mask = tf.where(tf.less(tf.random_uniform([batch_size], 0, 1, dtype=tf.float32), 0.5),
                                          tf.ones([batch_size, seq_length], dtype=tf.bool),
            ...
            for i in range(1, res_length):
                ...
                if i == 1:
                    l_mask = l1_mask
                    t_mask = tf.math.logical_or(t2_mask, tf.math.logical_or(t3_mask, t4_mask))
                elif i == 2:
                    l_mask = l1_mask
                    t_mask = t0_mask
                elif i == 3:
                ...
                # xx = tf.reduce_any(tf.math.logical_and(l0_ssr_mask, tf.math.logical_not(selected_mask)), axis=1)
                # ssr_mask=tf.where(xx, tf.tile(tf.math.logical_not(l0_ssr_mask)[tf.newaxis],[batch_size,1]), ones)
                # xx = tf.reduce_any(tf.math.logical_and(taohua_mask, tf.math.logical_not(selected_mask)), axis=1)
                # th_mask = tf.where(xx, tf.tile(tf.math.logical_not(taohua_mask)[tf.newaxis],[batch_size,1]), ones)
