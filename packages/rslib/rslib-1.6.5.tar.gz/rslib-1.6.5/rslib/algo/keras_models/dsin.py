...
def get_model(config):
    ...
    hidden_unit = config['dsin_hidden_units']
    emb_size = config['dsin_emb_size']
    ...
    seq_group_index = config['dsin_seq_group']
    target_seq = config['dsin_target_seq']
    ...
    a, b = DSIN(config=config)(dense_group_target, list_group_dense, mask=time_mask_all)

    all_feature = layers.Concatenate(axis=-1)([a, b, dense_cross_feature, user_feature, seq_index_layer([dense_group_target, 0])])
