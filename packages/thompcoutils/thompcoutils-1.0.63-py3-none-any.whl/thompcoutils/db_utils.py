...
class DbUtils:
    ...
    def create_table(self, table):
        ...
        if not self.table_exists(table):
            logger.debug("creating table {}".format(str(table.sqlmeta.table)))
            table.createTable()
            logger.debug("created table {}".format(str(table.sqlmeta.table)))
    ...
    def drop_tables(self, tables=None):
        ...
        if tables is None:
            tables = self.tables
        for table in tables:
            logger.debug("dropping table {}".format(str(table.sqlmeta.table)))
...
def _main():
    ...
    assert_test(not test_db.table_exists(_TestTable),
                message='Testing that table {} was successfully removed'.format(_TestTable.sqlmeta.table),
                error='Table {} should not exist'.format(_TestTable.sqlmeta.table),
                passed='Table {} removed successfully'.format(_TestTable.sqlmeta.table))
    test_db.create_tables()
    assert_test(test_db.table_exists(_TestTable),
                message='Testing that table {} was successfully created'.format(_TestTable.sqlmeta.table),
                error='Table {} should exist'.format(_TestTable.sqlmeta.table),
                passed='Table {} created successfully'.format(_TestTable.sqlmeta.table))
    _TestTable.validate()
    test_db.shutdown()
