...
class DeployHandler(Core):
    ...
    def handle(self):
        ...
        if app_project_root_folderpath is None:
            ...
            if not click.confirm(f"Do you want to deploy the project in this folder : {app_project_root_folderpath}"):
                app_project_root_folderpath = prompt_user_to_select_folderpath()
                changed_root_folderpath = True
        ...
        print(f"Saved your settings to your app_settings file at : {click.style(text=str(self.settings.last_settings_filepath), fg='yellow')}")
