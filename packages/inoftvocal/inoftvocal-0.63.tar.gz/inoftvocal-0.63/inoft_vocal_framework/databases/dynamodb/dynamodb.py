...
class DynamoDbAttributesAdapter(DynamoDbCoreAdapter):
    ...
    def _fetch_attributes(self, user_id: str) -> SafeDict:
        try:
            ...
            response = self.dynamodb.get_item(TableName=self.table_name, Key={"id": TypeSerializer().serialize(user_id)}, ConsistentRead=True)
            if "Item" in response:
                return SafeDict(self.utils.python_to_dynamodb(response["Item"]))
            else:
                return SafeDict()
