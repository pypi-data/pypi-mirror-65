...
class Settings:
    ...
    def find_load_settings_file(self, root_folderpath: str):
        ...
        for filenames in os.listdir(root_folderpath):
            for filenames in filenames:
                if filenames in ["app_settings.yaml", "app_settings.json"]:
                    found_settings_filepaths.append(os.path.join(root_folderpath, filenames))

        file_path_object = None
        if len(found_settings_filepaths) == 0:
            print("Did not found an app_settings.yaml or app_settings.json file."
        ...
        if not len(found_settings_filepaths) == 1:
