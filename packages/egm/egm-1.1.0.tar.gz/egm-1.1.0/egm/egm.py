...
def evidencegapmap(dataset, x_column, y_column, xy_column=None, bubble_column=None, bubble_text=None, bubble_link=None, time_column=None, size_column=None, color_column=None,   
    ...
    if xy_column :
        mode = 'nlpmode'
        xmax = int(max(map(lambda xy: xy[0], list(dataset[xy_column]))))
        xmin = int(min(map(lambda xy: xy[0], list(dataset[xy_column]))))
        ymax = int(max(map(lambda xy: xy[1], list(dataset[xy_column]))))
        ymin = int(min(map(lambda xy: xy[1], list(dataset[xy_column]))))
        xshift = (xmax + xmin)/2
        yshift = (ymax + ymin)/2
