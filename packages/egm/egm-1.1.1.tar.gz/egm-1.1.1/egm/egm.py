...
def evidencegapmap(dataset, x_column, y_column, xy_column=None, bubble_column=None, bubble_text=None, bubble_link=None, time_column=None, size_column=None, color_column=None,   
    ...
    if xy_column :
        mode = 'nlpmode'
        xmax = max(map(lambda xy: float(xy[0]), list(dataset[xy_column]))))
        xmin = min(map(lambda xy: float(xy[0]), list(dataset[xy_column]))))
        ymax = max(map(lambda xy: float(xy[1]), list(dataset[xy_column]))))
        ymin = min(map(lambda xy: float(xy[1]), list(dataset[xy_column]))))
        xshift = (xmax + xmin)/2
        yshift = (ymax + ymin)/2
