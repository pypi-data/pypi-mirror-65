...
class AppWebKnowledge(object):
    ...
    def choose_pub_year(self, year: int):
        # '//*[@id="PublicationYear_tr"]/button[1]'
        value = f'PublicationYear_{year}'
        try:
            self.driver.find_element_by_xpath("//a[@id='PublicationYear']").click()
            WebDriverWait(driver=self.driver, timeout=60, poll_frequency=0.5).until(
                expected_conditions.presence_of_element_located((By.TAG_NAME, "input")))
            if self.mode == 'slow':
                sleep(random.uniform(2, 4))
            self.driver.find_element_by_xpath(f"//input[@type='checkbox' and @value='{value}']").click()
            if self.mode == 'slow':
                sleep(random.uniform(1, 3))
            self.driver.find_element_by_xpath("//button[@title='Refine']").click()
            WebDriverWait(driver=self.driver, timeout=60, poll_frequency=0.5).until(
    ...
    def fetch_citation(self, _type='citing'):
        ...
        try:
            ...
            if self.mode == 'slow':
                sleep(random.uniform(4, 6))
            print(f'[{time()}]:\033[1;36m success\033[0m to fetch the citation page .')
        except Exception as e:
            print(f'[{time()}]:\033[1;31m fail\033[0m to fetch the citation page .\n{e}')
    ...
    def _wrapper(self, element_id, annotation, click_type, verbose):
        try:
    ...
    def _show_more_authors(self, verbose=True):
        # search more authors if exits
        self._wrapper(
            element_id='show_more_authors_authors_txt_label', annotation='show more authors',
            click_type=0, verbose=verbose)

    def _hide_more_authors(self, verbose=True):
        # search more authors if exits
        self._wrapper(
            element_id='hide_more_authors_authors_txt_label', annotation='hide more authors',
            click_type=0, verbose=verbose)

    def _show_research_id(self, verbose=True):
        # search ResearcherID and ORC-ID if exits
        self._wrapper(element_id='show_resc_blurb_link', annotation='show Researcher-ID and ORC-ID',
                      click_type=0, verbose=verbose)

    def _hide_research_id(self, verbose=True):
        # search ResearcherID and ORC-ID if exits
        self._wrapper(element_id='hide_resc_blurb_link', annotation='hide Researcher-ID and ORC-ID',
                      click_type=0, verbose=verbose)

    def _show_funding_text(self, verbose=True):
        # search funding text information if exits
        self._wrapper(element_id='show_fund_blurb_link', annotation='show funding text',
                      click_type=0, verbose=verbose)

    def _hide_funding_text(self, verbose=True):
        # search funding text information if exits
        self._wrapper(element_id='hide_fund_blurb_link', annotation='hide funding text',
                      click_type=0, verbose=verbose)

    def _show_more_fields(self, _type='show', verbose=True):
        try:
    ...
    def expand_all_fields(self, funding_text=True, verbose=True):
        self._show_more_authors(verbose=verbose)
        self._show_research_id(verbose=verbose)
        if funding_text:
            self._show_funding_text(verbose=verbose)
        self._show_more_fields(_type='show', verbose=verbose)
    ...
    def parse_doi(self, verbose=False):
        # parse < doi > field
        doi = ''
        try:
    ...
    def parse_title(self, verbose=False):
        # parse < title > field
        title = ''
        try:
    ...
    def parse_abstract(self, verbose=False):
        ...
        for field in ['citing_all', 'citing', 'cited']:
            ...
            try:
                other_core_field[field] = eval(other_core_field.get(field).replace(
                    'in All Databases', '').replace(',', '').strip())
    ...
    def parse_article(self, verbose=False, dump=True, **kwargs):
        ...
        title = self.parse_title(verbose=verbose)
        if not title:
            ...
            parsed_info.update(self.parse_id(verbose=verbose))
