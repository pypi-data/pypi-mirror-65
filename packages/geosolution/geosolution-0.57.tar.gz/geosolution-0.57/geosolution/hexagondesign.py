...
class hexagon_grid:
    ...
    def __init__(self,
        ...
        if hexagon_dis == True:
            final_hexa["geometry"] = final_hexa["geometry"].apply(lambda geom: geom.wkb)
            final_hexa = final_hexa.drop_duplicates(["geometry"])
            final_hexa["geometry"] = final_hexa["geometry"].apply(lambda geom: loads(geom))
            pdf = gpd.overlay(gdf, final_hexa, how = 'union')
            check_topology(pdf)
