...
def get_url(pushgateway_url, myelin_installation_namespace, port, job_name):
    return f"http://{pushgateway_url}.{myelin_installation_namespace}.svc.cluster.local:{port}/metrics/job/{job_name}/pod/"
...
class MetricClientKubernetes(object):
    ...
    def post_update(self, metric, value, job_name=None):
        job = job_name if job_name else self.task_id
        url = get_url(self.pushgateway_url, self.myelin_installation_namespace, self.port, job)
        internal_metric_name = self.metrics[metric]
        payload = "{} {}".format(internal_metric_name, value)
        res = requests.post(url, payload, headers={'Content-Type': 'application/octet-stream'})
        return res
