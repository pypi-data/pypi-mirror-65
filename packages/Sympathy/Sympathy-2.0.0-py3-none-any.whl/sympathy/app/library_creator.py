...
def create(source_dirs, temp, session):
    ...
    for library in source_dirs:
        if os.path.exists(library):
            if sympathy.api.library.is_old_style(library):
    ...
    installed_libraries = list(sympathy.api.library.available_libraries())

    for package in itertools.chain(
            installed_libraries, new_style_packages):
        issues = []
        try:
            library_dict = sympathy.api.library.to_dict(package)
            ok = True
    ...
    for library in old_style:
        ...
        if not os.path.isdir(library_):
            ...
            library_dict['nodes'] = sympathy.api.library.scan_files(
                os.path.join(library, 'Library'), 'node_*.py')
            library_dict['flows'] = sympathy.api.library.scan_files(
                os.path.join(library, 'Library'), 'flow_*.syx')
            library_dict['plugins'] = sympathy.api.library.scan_files(
                library, 'plugin_*.py')
