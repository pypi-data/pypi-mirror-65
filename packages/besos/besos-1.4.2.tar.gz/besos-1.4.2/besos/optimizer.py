...
def platypus_alg(
    besos_evaluator: AbstractEvaluator,
    ...
    **kwargs,
    ...
    problem = besos_evaluator.to_platypus()
    besos_evaluator.problem.pre_optimaization()
    alg: platypus.AbstractGeneticAlgorithm = algorithm(problem, *args, **kwargs)
    alg.run(evaluations)
    # TODO: Have a smarter default for parts (instead of 'all')
    return solutions_to_df(platypus.unique(alg.result), besos_evaluator.problem)
...
def _alg_t(algorithm):
    ...
    def template(besos_evaluator, evaluations=conf["evaluations"], *args, **kwargs):
        for kwarg, default in defaults.items():
            if kwarg not in kwargs or kwargs[kwarg] == "automatic":
                kwargs[kwarg] = default(besos_evaluator.to_platypus())
        return platypus_alg(besos_evaluator, algorithm, evaluations, *args, **kwargs)
