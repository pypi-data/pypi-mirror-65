...
from corpusflow import Document, Span, SpanSet, DocumentPattern, EntityPlaceholder
...
def read_markdown(fd) -> List[Document]:
    doc = CommonMarkDocument(fd)
                ...
                for element in sub_item.children:
                    ...
                    for span_set in element.children:
                        if isinstance(span_set, RawText):  # plain text
                            partial_token_list = span_set.content.split()
