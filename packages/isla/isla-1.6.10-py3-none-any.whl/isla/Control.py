...
class Control(object):
    ...
    def econ_calc(self, cost_elec='lcoe', cost_sub=0, print_out=True):
        ...
        proj_opex_cf = self.proj_opex/(
            (1+self.infl)**np.arange(self.yr_proj+1)
        )
        proj_opex_cf[0] = 0
        self.cost_otot += np.sum(proj_opex_cf)

        # get revenue
        rev_cf = self.algo.ld.enr_tot*self.lcoe/(
            (1+self.infl)**np.arange(self.yr_proj+1)
        )
        rev_cf[0] = 0
    ...
    def cashflow_plot(
        ...
        col_f = Patch(color=col_light('#000000', 0.6), label='Fuel')
        ...
        for cp, sub in zip(self.cf_dict, self.cf_dict.values()):
            ...
            if cp == 'Revenue':
                ...
                header += [
                    ...
                    cp.name_solid+' RepEx', cp.name_solid+' Fuel'
                ]

                # append to cost array
                for cf in [
                    sub['CapEx'], sub['OpEx'], sub['RepEx'], sub['Fuel']
                ]:
                    cost_arr = np.append(cost_arr, np.atleast_2d(cf).T, axis=1)
