...
class Fuel(object):
    ...
    def fl_use_calc(self, yr_proj, infl):
        ...
        if callable(self.cost):
            fc = self.cost(yr)*((1+self.fl_infl)/(1+infl))**yr
        else:
            fc = self.cost*((1+self.fl_infl)/(1+infl))**yr
    ...
    def fl_ret_calc(self, yr_proj, infl):
        ...
        if callable(self.cost_ret):
            fc = self.cost_ret(yr)*((1+self.fl_infl)/(1+infl))**yr
        else:
            fc = self.cost_ret*((1+self.fl_infl)/(1+infl))**yr
