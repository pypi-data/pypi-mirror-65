...
class Component(object):
    ...
    def opex_fix_calc(self, yr_proj, infl):
        ...
        if callable(self.opex_fix):
            opex = self.opex_fix(yr)/(1+infl)**yr
        else:
            opex = self.opex_fix/(1+infl)**yr
    ...
    def opex_use_calc(self, yr_proj, infl):
        ...
        if callable(self.opex_use):
            opex = self.opex_use(yr)/(1+infl)**yr
        else:
            opex = self.opex_use/(1+infl)**yr
    ...
    def repex_calc(self, yr_proj, infl, rep_per):
        ...
        if callable(self.repex):
            ...
            for i, rp in enumerate(rep_per):
                ...
                repex[rep_yr, i] = 1/(1+infl)**rep_yr*self.repex(rep_yr)
            ...
            for i, rp in enumerate(rep_per):
                rep_yr = np.floor(np.arange(0, yr_proj, rp))
                rep_yr = np.array(rep_yr, dtype=int)[1:]
                repex[rep_yr, i] = 1/(1+infl)**rep_yr*self.repex
            cost_r = self.size*repex
