...
class QueryBuilder():
  ...
  def get(self):
    ...
    if self._type == QueryType.SINGULAR:
      ...
      return instances
  def prettify_compile(self):
    _query = self.compile()
    tabs = 0
    pretty_query = ''
    for i, ch in enumerate(_query):
      # get context
      prev_ch = _query[i-1] if i > 0 else ''
      next_ch = _query[i+1] if i < len(_query) - 1 else ''
  ...
  def compile_plural_params(self):
    ...
    # apply orderBy param
    orderby_params = self.gql_param_dumps('orderBy', self._orderBy, True) if self._orderBy else ''
    # apply limit
  ...
  def gql_param_dumps(self, k, v, force_enum=False):
    ...
    if type(v) == float:
      return f'{k}: {v}'
    if type(v) == str:
      if (v.isupper() and not contains_digits(v) and len(v) > 3) or force_enum:
        # SCREAMING_CASE, this means it's an Enum, loose the quotes
        return f'{k}: {v}'
      else:
    ...
    if type(v) == dict:
      subparams = " ".join([self.gql_param_dumps(subk, subv, force_enum) for subk, subv in v.items()])
