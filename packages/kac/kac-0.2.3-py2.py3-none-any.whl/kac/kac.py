...
def template():
    ...
    if (len([filename for filename in os.listdir(os.getcwd()) if
        ...
        click.echo(f'{Changelog.default_file_name} already exists.')
        raise click.Abort
    ...
    github_repo_url: str = questionary.text(
        message='Enter Repository URL (ie. https://github.com/atwalsh/kac)'
    ).ask()
    if github_repo_url is None:
        raise click.Abort
    changelog_template = Template('templates/CHANGELOG.md')
    changelog_template.render(initial_version=first_v_num, repo_url=github_repo_url)