...
def to_categorical(labels):
    ...
    if labels.shape[-1] == 1:
        labels = cp.squeeze(labels, axis=-1)

    n_class = cp.max(labels).tolist() + 1
    one_hot = cp.eye(n_class)[labels]
    return one_hot



def concatenate(*variables, axis, output=None, name=None):
    new_shape = cp.asarray(variables[0].output_shape)
    new_value = variables[0].output_tensor
    for i in range(1,len(variables)):
        shape = cp.asarray(variables[i].output_shape)
