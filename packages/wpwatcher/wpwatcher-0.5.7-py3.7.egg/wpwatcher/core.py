...
class WPWatcher():
        ...
        if wpscan_exit_code not in [0,5]:
            ...
            if self.conf['fail_fast']:
                if not self.interrupting: 
                    log.error("Failure")
                    self.interrupt()
                # Handle the case where the interrupt() method caused WPScan to exeit with random codes and not 2
                else: return None 
    ...
    def interrupt(self, sig=None, frame=None):
        ...
        # Kill thread
        log.error(str(threading.current_thread()))
        # self.executor.shutdown(wait=True)
        # This code won't get executed if interrupt if called from a thread
    ...
    def run_scans_and_notify(self):
        ...
        for f in self.futures:
            ...
            # Runtime error is raised when calling interrupt() from inside a scan_site method in a thread
