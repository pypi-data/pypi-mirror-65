...
# We need symlinks for MacOS
if sys.platform == 'darwin':
    libdir = get_distutils_lib_path()
    for lib in ['libbasiclu', 'libipx', 'libhighs', 'highs_wrapper']:
        libfile = pathlib.Path(MODULE_NAME + '/' + libdir + '/' + lib + SO_SUFFIX + '.so')
        libfile.parent.mkdir(parents=True, exist_ok=True)
        libfile.touch()

        libfile.symlink_to(pathlib.Path(MODULE_NAME + '/' + lib + SO_SUFFIX + '.so'))
        #pathlib.Path(MODULE_NAME + '/lib' + lib + SO_SUFFIX).symlink_to(libfile)
