# 参考：https://github.com/ray-project/ray/blob/releases/0.7.4/python/ray/experimental/streaming/batched_queue.py
...
from skydl.ray.experimental.async_api_0_8_4 import as_future as thread_safe_client
from skydl.ray.worker_0_8_1 import global_worker as ray081_global_worker
...
def plasma_prefetch(object_id):
    ...
    local_sched_client = ray081_global_worker.raylet_client
...
def experimental_plasma_get(object_id):
    """Get an object directly from plasma without going through object table.
    Precondition: plasma_prefetch(object_id) has been called before.
    (ref issue: https://github.com/ray-proje ct/ray/issues/5624)
    ...
    参考：创建plasma_client https://github.com/ray-project/ray/blob/releases/0.7.4/python/ray/worker.py
    """
    # plasma_id = plasma.ObjectID(object_id)
    # plasma_client = thread_safe_client(
    #     plasma.connect(ray.worker.global_worker.node.plasma_store_socket_name, None, 0, 300))
    plasma_client = plasma.connect(ray.worker.global_worker.node.plasma_store_socket_name)
    plasma_id = plasma.ObjectID(object_id)
    while not plasma_client.contains(plasma_id):
        # print(f"experimental_plasma_get not contains plasma_id: {plasma_id}")
        pass
    print(f"ok, experimental_plasma_get contains plasma_id: {plasma_id}")
...
class BatchedQueue(object):
    ...
    def _flush_writes(self):
        with self.flush_lock:
            ...
            ray.worker.global_worker.put_object(self.write_buffer, ray.ObjectID(batch_id))
    ...
    def put_next(self, item):
        print(f"===put_next==={self.background_flush} and {self.flush_thread.is_alive()}")
        with self.flush_lock:
