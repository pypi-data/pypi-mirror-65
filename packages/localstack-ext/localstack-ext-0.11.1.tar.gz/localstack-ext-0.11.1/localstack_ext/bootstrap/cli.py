...
def import_auth():
 try:
  ...
  raise hGjMw('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=hGjMd)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=hGjML)
def get_params(args,names=[],mandatory=hGjMd):
 result=()
 for name in names:
  value=hGjMT
  if name in args:
  ...
  if not value and mandatory:
   raise hGjMw('Please provide %s=... parameter'%name)
  result+=(value)
 if hGjMf(result)==1:
  return result[0]
 return result
