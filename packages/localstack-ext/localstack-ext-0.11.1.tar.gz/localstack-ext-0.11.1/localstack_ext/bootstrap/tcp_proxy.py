...
class AttrDict(aSTBx):
 def __init__(self,*args,**kwargs):
  aSTBM(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and aSTBX(socket,'IPV6_V6ONLY'):
...
class Half(aSTBH):
 def __init__(self,opt,sock,addr,aSTBz):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.aSTBz=aSTBz
  self.name='peer' if self.aSTBz else 'client'
  self.queue=[]
  self.dest=aSTBe
  self.err=aSTBe
  self.ready=aSTBW
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':aSTBW,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
