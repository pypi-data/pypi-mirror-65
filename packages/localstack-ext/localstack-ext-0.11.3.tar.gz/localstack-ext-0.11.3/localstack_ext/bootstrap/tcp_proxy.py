...
class AttrDict(tQebN):
 def __init__(self,*args,**kwargs):
  tQebA(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and tQebm(socket,'IPV6_V6ONLY'):
...
class Half(tQebw):
 def __init__(self,opt,sock,addr,tQebd):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.tQebd=tQebd
  self.name='peer' if self.tQebd else 'client'
  self.queue=[]
  self.dest=tQebx
  self.err=tQebx
  self.ready=tQebK
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':tQebK,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
