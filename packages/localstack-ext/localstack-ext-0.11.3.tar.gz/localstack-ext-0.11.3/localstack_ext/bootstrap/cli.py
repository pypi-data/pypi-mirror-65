...
def import_auth():
 try:
  ...
  raise ELmqG('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=ELmqS)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=ELmqY)
def get_params(args,names=[],mandatory=ELmqS):
 result=()
 for name in names:
  value=ELmqv
  if name in args:
  ...
  if not value and mandatory:
   raise ELmqG('Please provide %s=... parameter'%name)
  result+=(value)
 if ELmqW(result)==1:
  return result[0]
 return result
