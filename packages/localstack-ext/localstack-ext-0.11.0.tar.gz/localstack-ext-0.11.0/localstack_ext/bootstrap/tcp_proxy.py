...
class AttrDict(eYFxX):
 def __init__(self,*args,**kwargs):
  eYFxz(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and eYFxH(socket,'IPV6_V6ONLY'):
...
class Half(eYFxw):
 def __init__(self,opt,sock,addr,eYFxL):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.eYFxL=eYFxL
  self.name='peer' if self.eYFxL else 'client'
  self.queue=[]
  self.dest=eYFxJ
  self.err=eYFxJ
  self.ready=eYFxS
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':eYFxS,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
