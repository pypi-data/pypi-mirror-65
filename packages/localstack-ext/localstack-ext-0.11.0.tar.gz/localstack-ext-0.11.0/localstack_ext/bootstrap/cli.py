...
def import_auth():
 try:
  ...
  raise GYWvV('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=GYWvH)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=GYWvx)
def get_params(args,names=[],mandatory=GYWvH):
 result=()
 for name in names:
  value=GYWvF
  if name in args:
  ...
  if not value and mandatory:
   raise GYWvV('Please provide %s=... parameter'%name)
  result+=(value)
 if GYWvU(result)==1:
  return result[0]
 return result
