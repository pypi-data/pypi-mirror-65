...
class AttrDict(KMfDE):
 def __init__(self,*args,**kwargs):
  KMfDv(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and KMfDF(socket,'IPV6_V6ONLY'):
...
class Half(KMfDc):
 def __init__(self,opt,sock,addr,KMfDr):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.KMfDr=KMfDr
  self.name='peer' if self.KMfDr else 'client'
  self.queue=[]
  self.dest=KMfDA
  self.err=KMfDA
  self.ready=KMfDd
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':KMfDd,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
