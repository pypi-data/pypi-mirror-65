...
class FuncThread(threading.Thread):
 def __init__(self,func,params=NgJoQ):
...
class RequestHandler(BaseHTTPRequestHandler):
 def do_POST(self):
  ...
  try:
   ...
   error_string=NgJoq(e)
   result=json.dumps({'error':error_string})
   self.send_response(500)
  self.send_header('Content-Length','%s'%NgJom(result)if result else 0)
  self.end_headers()
  if NgJom(result or ''):
 ...
 def handle_request(self):
  ...
  result=json.dumps(result)if NgJof(result,NgJoz)else result
  return result
 def read_content(self):
  if NgJob(self,'data_bytes'):
   return
  content_length=self.headers.get('Content-Length')
  self.data_bytes=self.rfile.read(NgJoH(content_length))
  self.request_json={}
  try:
...
def s3_download(request):
 ...
 target_file=os.path.join(tmp_dir,request.get('file_name')or 's3file.%s'%NgJoq(uuid.uuid4()))
...
def start_ssh_forward_proxy(options):
 ...
 host=LOCAL_BIND_ADDRESS_PATTERN.replace('*',NgJoq(NgJom(USED_BIND_ADDRESSES)+2))
...
def create_network_interface_alias(address,interface=NgJoQ):
 sudo_cmd='sudo'
 try:
...
def run_shell_cmd(command):
 try:
  ...
  error_string=NgJoq(e)
  if NgJof(e,subprocess.CalledProcessError):
...
def run(cmd):
 ...
 return to_str(subprocess.check_output(cmd,stderr=subprocess.STDOUT,shell=NgJov))
def log(*args):
 print(*args)
 sys.stdout.flush()
def is_mac_os():
 try:
  out=to_str(subprocess.check_output('uname -a',shell=NgJov))
...
def get_free_tcp_port():
 ...
 return port
def to_bytes(obj):
 return obj.encode('utf-8')if NgJof(obj,NgJoq)else obj
def to_str(obj):
 return obj.decode('utf-8')if NgJof(obj,NgJoR)else obj
...
def main():
 logging.basicConfig()
 daemon_type=sys.argv[1]if NgJom(sys.argv)>1 else 'main'
 if daemon_type=='main':
...
def start_in_background():
 ...
 run(cmd,outfile=log_file,asynchronous=NgJov)
