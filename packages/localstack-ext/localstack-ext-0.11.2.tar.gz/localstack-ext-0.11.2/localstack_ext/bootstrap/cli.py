...
def import_auth():
 try:
  ...
  raise pomae('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=pomaN)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=pomag)
def get_params(args,names=[],mandatory=pomaN):
 result=()
 for name in names:
  value=pomah
  if name in args:
  ...
  if not value and mandatory:
   raise pomae('Please provide %s=... parameter'%name)
  result+=(value)
 if pomaq(result)==1:
  return result[0]
 return result
