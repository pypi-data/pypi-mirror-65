...
class AttrDict(jIQHM):
 def __init__(self,*args,**kwargs):
  jIQHv(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and jIQHt(socket,'IPV6_V6ONLY'):
...
class Half(jIQHG):
 def __init__(self,opt,sock,addr,jIQHF):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.jIQHF=jIQHF
  self.name='peer' if self.jIQHF else 'client'
  self.queue=[]
  self.dest=jIQHT
  self.err=jIQHT
  self.ready=jIQHA
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':jIQHA,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
