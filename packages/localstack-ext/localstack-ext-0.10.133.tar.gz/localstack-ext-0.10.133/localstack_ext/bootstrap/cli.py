...
def import_auth():
 try:
  ...
  raise UrWMw('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=UrWMD)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=UrWMA)
def get_params(args,names=[],mandatory=UrWMD):
 result=()
 for name in names:
  value=UrWMI
  if name in args:
  ...
  if not value and mandatory:
   raise UrWMw('Please provide %s=... parameter'%name)
  result+=(value)
 if UrWMn(result)==1:
  return result[0]
 return result
