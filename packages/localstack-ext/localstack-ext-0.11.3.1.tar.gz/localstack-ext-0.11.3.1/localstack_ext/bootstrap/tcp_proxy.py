...
class AttrDict(DREkS):
 def __init__(self,*args,**kwargs):
  DREkX(AttrDict,self).__init__(*args,**kwargs)
  self.__dict__=self
def tcp_listen(six,addr,port,blk):
 s=socket.socket(AF_INET6 if six else AF_INET,SOCK_STREAM)
 if six and DREkr(socket,'IPV6_V6ONLY'):
...
class Half(DREkt):
 def __init__(self,opt,sock,addr,DREkq):
  self.opt=opt
  self.sock=sock
  self.addr=addr
  self.DREkq=DREkq
  self.name='peer' if self.DREkq else 'client'
  self.queue=[]
  self.dest=DREki
  self.err=DREki
  self.ready=DREks
 ...
 while qs:
  ...
  for q in qs:
   if q.post_wait(r,w,e):
    qs.remove(q)
def main():
 opt={'ip6':DREks,'bind_port':2223,'addr':'192.168.43.191','port':22}
 server_loop(opt)
