...
def import_auth():
 try:
  ...
  raise CKptc('Command not available in this version')
def mand_params(args,names=[]):
 return get_params(args,names,mandatory=CKptj)
def opt_params(args,names=[]):
 return get_params(args,names,mandatory=CKptd)
def get_params(args,names=[],mandatory=CKptj):
 result=()
 for name in names:
  value=CKptA
  if name in args:
  ...
  if not value and mandatory:
   raise CKptc('Please provide %s=... parameter'%name)
  result+=(value)
 if CKptm(result)==1:
  return result[0]
 return result
