...
class ImportTabularDataModule:
    ...
    def generate_dataflow(cls) -> 'Dataflow':
        ...
        try:
            ...
            elif "Operation returned an invalid status code 'Could not access the specified path due to permission " \
                 "or read-only properties. Failed to authorize w" in error_message:
                ErrorMapping.rethrow(exceptions, UserError(error_message))
            else:
                raise exceptions
