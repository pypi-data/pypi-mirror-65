...
The imagewiz module is deisgned to work with established image processing python modules.
This library is meant to be used to streamline the practical workflow of image preprocessing.
...
class fileWiz:
    ...
    def classGrab(df, pki_index_col, tar_class = 'class', verbose = False):
    ...
    def folderKing(df,
        ...
        for i in fileWiz.classGrab(df)[1]:
        ...
        filename_list_train = []
        for i in fileWiz.classGrab(train)[1]:
            for x in fileWiz.classGrab(train)[0][i]:
                filename_list_train.append(os.path.join(dest_folder,
                                                        train_folder_name,
                                                        str(i),
                                                        str(x) + extension_type))
    
        for i in fileWiz.classGrab(val)[1]:
            for x in fileWiz.classGrab(val)[0][i]:
                filename_list_train.append(os.path.join(dest_folder,
                                                        val_folder_name,
                                                        str(i),
                                                        str(x) + extension_type))
        ...
        filename_list_orig = []
        for f in fileWiz.classGrab(train)[1]:
            for y in fileWiz.classGrab(train)[0][f]:
                filename_list_orig.append(os.path.join(orig_folder,
                                                        str(y) + extension_type))      
            for y in fileWiz.classGrab(val)[0][f]:
                filename_list_orig.append(os.path.join(orig_folder,
                                                        str(y) + extension_type))  
