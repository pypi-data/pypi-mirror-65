...
static int tcp_open_socket(TCPDeviceInfo_t* tcp_info) // call with both poll_list_mutex and device lock
{
	...
	if (tcp_info->sockaddr.ss_family == AF_INET6)
	{
		socklen = sizeof(struct sockaddr_in6);
	}
	else if (tcp_info->sockaddr.ss_family == AF_INET)
	{
	...
	else
	{
		...
		socklen = sizeof(tcp_info->sockaddr);
	}

	ret = connect(tcp_info->fd, (struct sockaddr*)&tcp_info->sockaddr, socklen);
	if (ret != 0)
	{
#ifdef _WIN32
		int error = WSAGetLastError();
...
static int tcp_reconnect_device(AsphodelDevice_t* device, AsphodelDevice_t** reconnected_device)
{
	...
	ret = sendto(fd, send_msg, sizeof(send_msg), 0, (struct sockaddr*)&dest_addr, dest_addr_len);
	if (ret < 0)
	{
		ret = tcp_last_error();
		TCP_CLOSE(fd);
...
static int tcp_send_multicast_packet(int socket_family, TCP_SOCKET_TYPE fd)
{
	...
	else if (dwRetVal == ERROR_ADDRESS_NOT_ASSOCIATED || dwRetVal == ERROR_NO_DATA)
	{
		...
		{
			DWORD value = 0;
			ret = setsockopt(fd, IPPROTO_IP, IPV6_MULTICAST_IF, (void*)&value, sizeof(value));
			if (ret != 0)
			{
				ret = tcp_last_error();
				return ret;
	...
	for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next)
	{
		...
		else if (socket_family == AF_INET6)
		{
			...
			ret = setsockopt(fd, IPPROTO_IP, IPV6_MULTICAST_IF, (void*)&ifindex, sizeof(ifindex));
			if (ret != 0)
			{
				ret = tcp_last_error();
				freeifaddrs(ifaddr);
...
ASPHODEL_API int asphodel_tcp_find_devices(AsphodelDevice_t** device_list, size_t* list_size)
{
	...
	// send the inquiry packet (will implicitly bind to INADDR_ANY)
	ret = tcp_send_multicast_packet(AF_INET, fd);
	if (ret != ASPHODEL_SUCCESS)
	{
		TCP_CLOSE(fd);
		return ret;
	...
	while (1)
	{
		...
		else
		{
			...
			else
			{
				...
				if (count < *list_size)
				{
					// check for duplicates already in the list (can happen with two NICs attached to the same network)
					size_t i;
					int found = 0;
					for (i = 0; i < count; i++)
					{
						...
						TCPDeviceInfo_t* tcp_info = (TCPDeviceInfo_t*)device->implementation_info;
						if (memcmp(&tcp_info->sockaddr, &recv_addr, addrlen) == 0)
						{
							found = 1;
							break;
						}
