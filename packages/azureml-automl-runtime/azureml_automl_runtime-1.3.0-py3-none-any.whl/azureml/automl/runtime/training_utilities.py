...
def incremental_learning_validate_and_subsample_inputs(
    ...
    if not isinstance(X, dprep.Dataflow):
        raise InvalidDataTypeException(
            input_must_be_dataflow_warning.format('X'), target="X",
            reference_code="af8b201d-f876-4a89-a229-faa6ead598c0", has_pii=False)
    if not isinstance(y, dprep.Dataflow):
        raise InvalidDataTypeException(
            input_must_be_dataflow_warning.format('y'), target="y",
            reference_code="d7535edc-f64c-4eb0-b352-0f6c1b6f35f3", has_pii=False)
    if X_valid is not None and not isinstance(X_valid, dprep.Dataflow):
        raise InvalidDataTypeException(
            input_must_be_dataflow_warning.format('X_valid'), target="X_valid",
            reference_code="4db02e89-ccf5-4dab-9f45-d9de48a7771f", has_pii=False)
    if y_valid is not None and not isinstance(y_valid, dprep.Dataflow):
        raise InvalidDataTypeException(
            input_must_be_dataflow_warning.format('y_valid'), target="y_valid",
            reference_code="5fa6ed3b-4c41-4267-aa03-d92905002ecf", has_pii=False)
    if sample_weight is not None and not isinstance(sample_weight, dprep.Dataflow):
        raise InvalidDataTypeException(
            input_must_be_dataflow_warning.format('sample_weight'), target="sample_weight",
            reference_code="6f17fb5d-9000-48d8-b061-60880e3bd7b6", has_pii=False)
    if sample_weight_valid is not None and not isinstance(sample_weight_valid, dprep.Dataflow):
        raise InvalidDataTypeException(input_must_be_dataflow_warning.format(
            'sample_weight_valid'), target="sample_weight_valid",
            reference_code="c4a8c978-1f5e-4289-8867-a418cde22a8c", has_pii=False)
...
def _check_datatypes(training_data: dprep.Dataflow, validation_data: dprep.Dataflow) -> None:
    if training_data is not None and validation_data is not None:
        train_dtypes = (training_data.take(LargeDatasetLimit.VALIDATION_SUBSAMPLE_SIZE).
                        to_pandas_dataframe().dtypes.items())
        valid_dtypes = (validation_data.take(LargeDatasetLimit.VALIDATION_SUBSAMPLE_SIZE).
                        to_pandas_dataframe().dtypes.items())
