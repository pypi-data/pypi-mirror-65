...
class Monitaur:
    ...
    def record_training_image(
        ...
        if ("library" in model and model["library"] != "pytorch") or (
            "model" in model and model["model_type"] != "convolutional_neural_network"
        ):
            print(f"Training not recorded.")
            print(
                f"ML library must be PyTorch and model type must be convolutional_neural_network."
            )
            return False
        ...
        model_filename = f"{model_set_id}.pth.tar"
        ...
        with open(trained_model, "rb") as f:
            client.upload_fileobj(
                f,
                credentials["aws_bucket_name"],
                f"{model_set_id}/{version}/{model_filename}",
            )
