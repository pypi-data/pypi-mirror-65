        ...
        if optimize_top_adjacent:
            def __iterate(G):
                for lower in sma.sesSubgraph(G, level2):
                    top = sma.sesSubgraph(G, level0, G[lower])
...
class MultiMotifs(SourceMotifIterator):
    ...
    def __str__(self):
        return ('MultiMotifs[' + ','.join(['%d'] * len(self.arities)) + ']') % self.arities
...
class is4Type(ConditionMotifIterator):
    """
    Condition for 4-motifs modelling that the motif is of certain types, cf.
    :py:meth:`sma.classify4Motif`.
    
    This condition works only for 4-motifs.
    
    :param typ: types of the motifs to look for, e.g. 'II.C'.
    """
    def __init__(self, *typ):
        self.typ = typ
        super().__init__(lambda G, motif : sma.classify4Motif(G, motif) in typ)
    def __str__(self):
        return 'has4Type[%s]' % self.typ
        
class is3Type(ConditionMotifIterator):
    """
    Condition for 3-motifs modelling that the motif is of certain types, cf.
    ...
    def __str__(self):
        return 'has3Type[%s]' % self.typ

class isType(ConditionMotifIterator):
    """
    Condition for any sort of motif modelling that a motif is of a certain type.
    Provide a :py:class:`sma.MotifClassificator` as first parameter. The classification
    generated by this classificator will be used to match the given types.
    
    :param classificator: a :py:class:`sma.MotifClassificator` for classifying
        the motifs
    :param typ: one or several types
    """
    ...
    def __str__(self):
        return 'hasType[%s=%s]' % (str(self.classificator), self.typ)
