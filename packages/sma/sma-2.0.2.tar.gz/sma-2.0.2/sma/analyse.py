...
def countMotifs(G : nx.Graph, 
    ...
    """
    result = {key : 0 for key in classificator.names}
    ...
    # output format    
    if array:
        return numpy.array([result[k] for k in classificator.names])
...
def countMultiMotifs(G : nx.Graph, 
    ...
    **Speed up mode:** Due to its complexity, :py:class:`sma.MultiMotifClassificator`
    is quite slow and cannot be pickled. This means that in cannot be used in 
    multi-process mode. Setting the ``speedup`` parameter to ``True`` enables the
    following mechanism: If the given set of arities is supported (see below), 
    :py:class:`sma.MultiMotifClassificator` is replaced with a simpler classificator,
    e.g. :py:class:`sma.ThreeMotifClassificator`. Likewise the motif iterator is 
    replaced (set ``iterator`` to your favorite iterator to prevent this). In speed
    up mode, one can set ``processes = 6`` to run the counting with 6 concurrent 
    processes, say.
    ...
    In speed up mode execution times should be close to the execution times when
    using functions like :py:meth:`sma.count4Motifs` directly. See the following
    example done for a SEN with 22 vertices and 55 edges:
        
    .. code :: Python
    
        %timeit sma.count4Motifs(G)
        # 100 loops, best of 3: 9.51 ms per loop
        
        %timeit sma.countMultiMotifs(madagascar, 2,2, roles=[1,0])
        # 100 loops, best of 3: 11.6 ms per loop
        
        %timeit sma.countMultiMotifs(madagascar, 2,2, roles=[1,0], speedup = True)
    ...
    :param speedup: whether to use speed up mode, see above
    ...
    if speedup:
        ...
        if signature == [1,2]:
            positions = sma.classify._matchPositions([1,2], arities, roles)
            if iterator is None:
                iterator = iterate3Motifs(G, positions[0], positions[1])
            return count3Motifs(G, iterator, **kwargs)
        elif signature == [2,2]:
            positions = sma.classify._matchPositions([2,2], arities, roles)
            if iterator is None:
                iterator = iterate4Motifs(G, positions[0], positions[1])
            return count4Motifs(G, iterator, **kwargs)
        multis = [
                # signature, ordered,  classificator
                ([1, 1, 1], [1, 1, 1], sma.Multi111MotifClassificator),
                ([1, 1, 2], [1, 2, 1], sma.Multi121MotifClassificator),
                ([1, 2, 2], [2, 2, 1], sma.Multi221MotifClassificator),
                ([2, 2, 2], [2, 2, 2], sma.Multi222MotifClassificator),
            ]
        for sig, sig_ordered, classificator in multis:
            if signature == sig:
                positions = sma.classify._matchPositions(sig_ordered, arities, roles)
                if iterator is None:
                    iterator = sma.MultiMotifs(G, *sig_ordered, flatten = True, subsystems = positions)
                return countMotifs(G, classificator(G), iterator, **kwargs)
        print("Warning: Speed up for signature", signature, "failed, falling back to slow version.")
    
    if iterator is None:
        iterator = sma.MultiMotifs(G, *arities)
    return countMotifs(G, sma.MultiMotifClassificator(G, *arities, roles = roles), iterator, **kwargs)
...
def count3MotifsSparse(G : nx.Graph, 
    ...
    types of 3-motifs. Let :math:`m \in \mathbb{N}^6` be a row vector denoting the numbers of
    occurences of the motif types. Let :math:`C \in \mathbb{N}^{6 \\times 2}` denote 
    the matrix in :py:const:`sma.MOTIF3_EDGES` containing one row for every motif
    type with two entries each. The first entry represents the number of ``level1``
    ...
    H = G.subgraph(filter(lambda x : G.node[x]['sesType'] == level0 or G.node[x]['sesType'] == level1, G.nodes))
    ...
    counts = result_array @ sma.properties.MOTIF3_EDGES
    V = sma.nodesCount(H)
    
    # total = numpy.array([V[level0] * edges_count11, (V[level1]-1) * edges_count01])
...
def count4MotifsSparse(G : nx.Graph, 
    ...
    **Computing the numbers of motifs IV.A-IV.D:** This function iterates over
    ...
    all motifs except for the types IV.A to IV.D. Based on the double counting 
...
def count121MotifsSparse(G : nx.Graph,
    ...
    With default parameters, this function counts motifs of all types correctly.
    If ``optimize_top_adjacent = True``, only types ``3`` and ``4`` are counted correctly.
    ...
    :param optimize_top_adjacent: whether the counting should be optimized for
        counting motifs of types ``3`` and ``4``, see above.
    :param debug_unclassified: whether, in case that ``optimize_top_adjacent = True``,
        the partial count for type ``-1`` should be returned. Per default, the 
        value is set to a negative value.
    """
    classificator = sma.Multi121MotifClassificator(G)
    result = {k : 0 for k in classificator.names}
    for lower in sma.sesSubgraph(G, level2):
        if optimize_top_adjacent:
            top = filter(lambda x: G.node[x]['sesType'] == level0, G[lower])
        else:
            top = sma.sesSubgraph(G, level0)
        middle = filter(lambda x: G.node[x]['sesType'] == level1, G[lower])
    ...
    if optimize_top_adjacent:
        ...
        n0 = len(list(sma.sesSubgraph(G, level0)))
        n1 = len(list(sma.sesSubgraph(G, level1)))
        n2 = len(list(sma.sesSubgraph(G, level2)))
...
def count221MotifsSparse(G : nx.Graph,
    ...
    types *CLASS*.3 are counted correctly. 
    
    With default parameters, only motifs of types *CLASS*.2 and *CLASS*.3 are counted
    ...
    :param array: whether the output should be an array or a dict
    :param optimize_top_adjacent: whether the counting should be optimized for
        counting motifs of types *CLASS*.3, see above.
    :param processes: number of processes for multiprocessing
    :param chunksize: chunksize for multiprocessing
    :param debug_unclassified: whether, the partial count for type ``Unclassified``
...
def count222MotifsSparse(G : nx.Graph,
    ...
    This function counts only motifs of types ``3`` and ``4`` correclty. All other
    ...
    :param processes: number of processes for multiprocessing
    :param chunksize: chunksize for multiprocessing
    :param debug_unclassified: whether, the partial count for type ``-1``
        should be returned. Per default, the value is set to a negative value.
    """
...
def countMultiMotifsSparse(G : nx.Graph,
    ...
    multis = [
                ...
                ([1, 2],    [1, 2],    sma.count3MotifsSparse),
                ([2, 2],    [2, 2],    sma.count4MotifsSparse),
                ([1, 1, 2], [1, 2, 1], sma.count121MotifsSparse),
                ([1, 2, 2], [2, 2, 1], sma.count221MotifsSparse),
                ([2, 2, 2], [2, 2, 2], sma.count222MotifsSparse),
            ]
    ...
    raise TypeError('No suitable sparse counter found')

# Iterators
    
def iterate4Motifs(G : nx.Graph, 
                   level0 = sma.properties.NODE_TYPE_SOC, 
                   level1 = sma.properties.NODE_TYPE_ECO):
    """
...
def iterate3EMotifs(G : nx.Graph,
                    level0 = sma.properties.NODE_TYPE_ECO,
                    level1 = sma.properties.NODE_TYPE_SOC):
    """
...
def sesSubgraph(G : nx.Graph, sesType : int, source = None) -> filter:
    ...
    if source == None:
        source = G.nodes
    return filter(lambda x : G.node[x]['sesType'] == sesType, source)
...
def edgesCount(G : nx.Graph, array : bool =False) -> dict:
    ...
    def _classify(edge):
        ...
        if types == [sma.properties.NODE_TYPE_SOC, sma.properties.NODE_TYPE_SOC]:
            return sma.properties.EDGE_TYPE_SOC_SOC
        if types == [sma.properties.NODE_TYPE_ECO, sma.properties.NODE_TYPE_ECO]:
            return sma.properties.EDGE_TYPE_ECO_ECO
        return sma.properties.EDGE_TYPE_ECO_SOC
    classification = list(map(_classify, G.edges))
    types, counts = numpy.unique(classification, return_counts=True)
    if array:
        return [int(counts[types == k][0]) if k in types else 0 for k in [sma.properties.EDGE_TYPE_SOC_SOC, sma.properties.EDGE_TYPE_ECO_SOC, sma.properties.EDGE_TYPE_ECO_ECO] ]
    else:
        return { k : int(counts[types == k][0]) if k in types else 0 for k in [sma.properties.EDGE_TYPE_SOC_SOC, sma.properties.EDGE_TYPE_ECO_SOC, sma.properties.EDGE_TYPE_ECO_ECO] }
...
def total4Motifs(G : nx.Graph, multi : bool = False) -> int:
    """
    ...
    You must switch the parameter ``multi`` on if you want to use this function
    with multilevel networks, cf. :py:meth:`sma.totalMultiMotifs`.
    
    :param G: the graph
    :param multi: whether the network is a multilevel network (default false)
    """
    ecoNodes = len(list(sma.analyse.sesSubgraph(G, sma.properties.NODE_TYPE_ECO)))
    if multi:
        socNodes = len(list(sma.analyse.sesSubgraph(G, sma.properties.NODE_TYPE_SOC)))
    else:
        socNodes = len(G.nodes) - ecoNodes
    return ecoNodes * (ecoNodes -1) * socNodes * (socNodes -1) // 4

def total3EMotifs(G : nx.Graph, multi : bool = False) -> int:
    """
    ...
    if multi:
        ...
        socNodes = G.number_of_nodes() - ecoNodes
    return ecoNodes * socNodes * (socNodes -1) // 2

def total3SMotifs(G : nx.Graph, multi : bool = False) -> int:
    """
    ...
    if multi:
        ...
        socNodes = G.number_of_nodes() - ecoNodes
    return socNodes * ecoNodes * (ecoNodes -1) // 2
...
def totalMultiMotifs(G : nx.Graph, *arities : int) -> int:
    ...
    for level, arity in enumerate(arities):
        if arity < 1:
            continue
        N = len(list(sma.analyse.sesSubgraph(G, level)))
