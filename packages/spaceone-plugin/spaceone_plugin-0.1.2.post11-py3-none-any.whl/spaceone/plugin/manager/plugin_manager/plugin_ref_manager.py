...
class PluginRefManager(BaseManager):
    ...
    def list(self, query):
        return self._installed_plugin_ref_model.query(**query)
    ...
    def search_plugin(self, supervisor_id, plugin_id, version, state='ACTIVE'):
        ...
        try:
            ...
            search_plugin_param = _make_search_plugin_param(supervisor_id, plugin_id, version, state)
    ...
    def install_plugin(self, supervisor, installed_plugin, params):
        ...
        #installed_plugin_ref.update({'plugin_owner': installed_plugin})
        return installed_plugin_ref

def _make_search_plugin_param(supervisor_id, plugin_id, version, state):
    return {
        'filter': [
