...
class PluginManager(BaseManager):
    ...
    def delete(self, supervisor_id, plugin_id, version, domain_id):
        """ Delete Supervisor
        """
        install_plugin = self.get(supevisor_id, domain_id, plugin_id, version)
    ...
    def list(self, query):
        return self._installed_plugin_model.list(**query)

    def search_plugin(self, supervisor_id, plugin_id, version, state='ACTIVE'):
        """ Get installed_plugin
        """
        try:
            return self._installed_plugin_model.get(supervisor_id = supervisor_id,
                                            plugin_id = plugin_id,
                                            version = version,
                                            state = state)
        except Exception as e:
            _LOGGER.debug(f'[search_plugin] at PluginManager not found {supervisor_id}, {plugin_id}, {version}, {state}')
            return None
