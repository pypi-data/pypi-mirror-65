...
class CrossValidateModelModule(BaseModule):
    ...
    def cross_validate_generic(cls, learner: BaseLearner, training_data: DataTable,
        ...
        for fold, test_index in enumerate(fold_indices):
            ...
            # pass sorted test index because test index is set type.
    ...
    def format_score_results(fold_score_result_dict, meta_data, learner):
        ...
        dfs = [df.set_index(pd.Int64Index(idx)) for _, (df, idx) in fold_score_result_dict.items()]
        folds = [pd.Series(fold, index=pd.Int64Index(idx)) for fold, (_, idx) in fold_score_result_dict.items()]
