...
from azureml.studio.core.data_frame_schema import DataFrameSchema, ColumnTypeName
...
class EvaluateModelModule(BaseModule):
    ...
    def _validate_data_table(cls, data_table: DataTable, task_type, dataset_name):
        ...
        # dtype of label_column in regression task should be Numeric
        if task_type == TaskType.Regression and data_table.get_column_type(label_column_name) != ColumnTypeName.NUMERIC:
            error_setting.ErrorMapping.throw(error_setting.NotExpectedLabelColumnError(
                dataset_name=data_table.name,
                column_name=label_column_name,
                reason=f'The label column of Regression model should be of type "Numeric" but '
                       f'got "{data_table.get_column_type(label_column_name)}"'
            ))
