    ...
    *,
    ...
    if func is None:
        ...
        def wrapper(
            ...
            key = bound_args.sha512

            def compute_and_cache(desc: str) -> Any:
                out = cache[key] = wrapped(*args, **kwargs)
                LOGGER.debug(f"{desc} and cached: {bound_args} -> {out}")
                return out
...
class BoundArguments(inspect.BoundArguments):
    ...
    @property
    def sha512(self: BoundArguments) -> str:
        return sha512(bytes(self)).hexdigest()
