...
class interactive_aberration(object):
    """    
    ...
    def __init__(self,  horizontal = True):
        ...
        words = ['ideal rays','aberrated rays','aberrated wavefront','aberration function']
    ...
    def update(self):
        ...
        if words[3] in selection:
            ...
            x=np.append(x[::-1],x2)
            y=np.append(y[::-1],y2)
            aber = patches.Polygon(np.array([x+9,y]).T,
                        fill = True,
                        color='green', alpha = .5)

            aber2 = patches.Polygon(np.array([np.append(np.abs(dif),[0,0])*2+2.5 , np.append(np.linspace(-3.3,3.3,len(dif)),[3.3,-3.3])]).T,
                        fill = True,
                        color='green', alpha = .9)


            plt.gca().add_patch(aber)
            plt.gca().add_patch(aber2)    
