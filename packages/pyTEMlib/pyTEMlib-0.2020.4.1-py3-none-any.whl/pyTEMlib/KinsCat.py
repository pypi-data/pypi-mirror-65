...
def get_crystal_structure(crystal_name):
    """
    Return the crystal structure either from the database of KinsCat or a data file
    ...
    ------
        ...
        crystal_name: can be any string in the database
        crystal_name: can be 'POSCAR' to read a vasp input file: POSCAR
        crystal_name: can be 'cif' to read a cif structure file
    ...
    Usage:
    -------
        tags = get_crystal_structure['Si']
        tags = get_crystal_structure['POSCAR']
        tags = get_crystal_structure['cif']
"""
    if crystal_name in crystal_data_base:
        tags = structure_by_name(crystal_name)
    elif 'POSCAR' in crystal_name:
        tags = read_POSCAR()
    else:
        tags = read_cif()
...
def read_POSCAR(file_name = None, verbose = False):
    """
    Read a POSCAR file, an open file dialog will open if no file name is provided
...
def read_cif(file_name = None, verbose = False): 
    
    """Read a cif structure file, an open file dialog will open if no file name is provided
    
    Returns a dictionary 
    """
    if file_name == None:
        file_name = ft.openfile_dialog('cif (*.cif);;All files (*)')
    #use ase package to read file
...
class interactive_Bragg(object):
    """    
    ### Interactive explanation of Bragg's law and Ewald spheres
    ...
    def __init__(self,  horizontal = True):
        ...
        self.words = ["Bragg\'s law",'wave vectors','reciprocal lattice','Ewald sphere']
    ...
    def update(self):
        ...
        planes = np.array([[0,10],[0,0],[2,-2], [2,8]])
        crytal_planes_2D  = patches.Polygon(planes, \
                    edgecolor = 'blue', closed=True, fill = True, alpha = .5)   
        ax.add_patch(crytal_planes_2D)
        crytal_planes_2D  = patches.Polygon(planes+[1,1], \
                    edgecolor = 'blue', closed=True, fill = True, alpha = .5)   
        ax.add_patch(crytal_planes_2D)

        plt.text(2.4,-1.5,s= 'd')
        d_left2D = plt.arrow(1.,-2., .7, 0.4, width = .05)
        ax.add_patch(d_left2D)
        d_right = plt.arrow(3.8,-.1, -.7, -0.4, width = .05)
        ax.add_patch(d_right)

        plt.plot([5,5], [10,0], color = 'blue')
        plt.plot([6,6], [10,0], color = 'blue')
        
        plt.text(5.3,-0.5,s= 'd')
        d_left = plt.arrow(4,.2, .7, 0, width = .05)
        ax.add_patch(d_left)
        d_right = plt.arrow(7,.2, -.7, 0, width = .05)
        ax.add_patch(d_right)
        
        plt.text (5,-2, s='real space')
        plt.text (15,-2, s='reciprocal space')
        ...
        if self.words[0] in selection:
            plt.plot([8,5], [10,5], color = 'red')
            plt.plot([9,6], [10,5], color = 'red')
            plt.plot([8,5], [0,5], color = 'orange')
            plt.plot([9,6], [0,5], color = 'orange')
            ...
            Bragg_angle = patches.Arc([6,6.7],5,5, angle=60.0, theta1=0.0, theta2=30.0)
            ax.add_patch(Bragg_angle)
            plt.text(6.2,8,s= r'$\theta$')

            plt.plot([6,5.3],[5,4.5], color = 'black', linestyle = '--')
            #plt.plot([6,5.3],[5,5.5], color = 'black', linestyle = '--')

            plt.plot([5.3,4.6],[4.5,4], color = 'black', linestyle = '-')
            plt.plot([5,4.3],[5,4.5], color = 'black', linestyle = '-')

            plt.text(3,3.3,s= r'd $\sin \theta$', rotation=30,  fontsize=8)
            d_left = plt.arrow(4,.2, .7, 0, width = .05)
            ax.add_patch(d_left)
            d_right = plt.arrow(7,.2, -.7, 0, width = .05)
            ax.add_patch(d_right)
        ...
        if self.words[1] in selection:
            ...
            K_0 = plt.arrow(18,10,-(3-0.1),-(5-0.2), width = .05, color = 'red')
            K_d = plt.arrow(18,10, (3-0.1),-(5-0.2), width = .05, color = 'orange')
            K_d2 = plt.arrow(15,5, (3-0.1),-(5-0.2), width = .05, color = 'orange', linestyle = '--')

            plt.text(15.5, 7.5, s=r'$\vec{K}_0$')
            plt.text(19.6, 7.5, s=r'$\vec{K}_d$')
            plt.text(15.5, 2., s=r'$\vec{K}_d$')
            Bragg_angle = patches.Arc([15,5],4,4, angle=60.0, theta1=0.0, theta2=30.0)
            ax.add_patch(Bragg_angle)
            plt.text(15.1,6.,s= r'$\theta$')
            plt.plot([15,15],[8,5], color = 'gray', linestyle = '--')


            
        if self.words[2] in selection:
            
            spot = patches.Circle([15,5],.3, fill = True, facecolor= 'blue' , alpha = 0.5 , edgecolor = 'black')
            ax.add_patch(spot)
            spot = patches.Circle([21,5],.3, fill = True, facecolor= 'blue' , alpha = 0.5 , edgecolor = 'black')
            ax.add_patch(spot)
            plt.annotate(s='', xy=(15,5), xytext=(21,5), arrowprops=dict(arrowstyle='<->'), color = 'blue' )

            plt.annotate(s='', xy=(15,3.5), xytext=(21,3.5), arrowprops=dict(arrowstyle='<->'))
            plt.text(17.5,2.5, s = '1/d')
            plt.text(17.7,5.5, s = 'g')
        ...
        if self.words[3] in selection:
            ...
            spot = patches.Circle([18,10],.2, fill = True, facecolor= 'red' , alpha = 0.5 , edgecolor = 'black')
            ax.add_patch(spot)
            Ewald_sphere = patches.Circle([18,10],np.sqrt(5**2+3**2), fill = False)
            ax.add_patch(Ewald_sphere)

            Bragg_angle = patches.Arc([18,10],5,5, angle=0.0, theta1=240.0, theta2=300.0)
            ax.add_patch(Bragg_angle)
            plt.text(18.2,8,s= r'$\theta$')
            plt.text(17.3,8,s= r'$\theta$')
            plt.plot([18,18],[10,7], color = 'gray', linestyle = '--')

            plt.annotate(s='', xy=(18,10), xytext=(23.6,8), arrowprops=dict(arrowstyle='<->'))
            plt.text(20,8.8,s=r'r = $1/\lambda$', rotation =-19)
