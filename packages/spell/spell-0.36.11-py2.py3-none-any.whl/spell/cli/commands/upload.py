    ...
    try:
        ...
        if e.status_code:
            if e.status_code == 423:
                msg = ("A file appears to be locked. This could be the result of running 'spell upload' "
                       "on the same set of files. \n\n{}\n\nIf you are getting this message in error "
                       "please contact Spell support to get this upload unlocked.").format(resume_message)
        raise ExitException(msg or click.wrap_text("Unexpected upload issue: {}. {}".format(e, resume_message)))
    except TusCommunicationError as e:
        msg = None
        if e.status_code:
            if e.status_code == 409:
                msg = ("A file appears to have been modified since a previous upload of "
                       "'{}' was initiated. Please restore the upload to its original contents or initiate a "
                       "new upload under a different name. {}").format(name, resume_message)
            if e.status_code == 423:
                msg = ("A file appears to be locked. This could be the result of running 'spell upload' "
                       "on the same set of files. \n\n{}\n\nIf you are getting this message in error "
                       "please contact Spell support to get this upload unlocked.").format(resume_message)
