    ...
    if len(run_ids) <= 0:
        ...
        if not quiet:
            if len(data["still_active_run_ids"]) > 0:
                click.echo("| Still active: {}".format(" ".join([str(i) for i in data["still_active_run_ids"]])))
            ...
            if len(data["successful_run_ids"]) > 0:
                click.echo("| Successfully archived: {}".format(" ".join([str(i) for i in data["successful_run_ids"]])))
            if len(data["failed_run_ids"]) > 0:
                logger.error("Failed to archive: {} (For more information, contact Spell)"
