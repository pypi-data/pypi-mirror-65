        ...
        for spec_api in self.input_spec["paths"]:
            ...
            if url_pattern_checked is not None:
                ...
                for api_method in self.input_spec["paths"][spec_api]:
                    ...
                    if respcode_inspec and len(set(respcode_inspec)-set(respcode_inspec)) == 0:
                    ...
                    if self.input_spec["paths"][spec_api][api_method]["parameters"]:
                        ...
                        if param_stats_conditional.count("m_missing") > 0:
                            ...
                            overall_api_stats["cumulative_total_mandatory_missing"]+=param_stats_conditional.count("m_missing")
                        if param_stats_conditional.count("o_missing") > 0:
                            overall_api_stats["apis_missing_optional"].add(spec_api)
                            overall_api_stats["cumulative_total_optional_missing"] += param_stats_conditional.count("o_missing")
                    else:
