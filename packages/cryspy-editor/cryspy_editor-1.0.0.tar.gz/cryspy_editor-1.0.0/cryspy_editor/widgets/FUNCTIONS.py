...
def make_qtablewidget_for_data_constr(obj, label=None):
    ...
    w_t.setVerticalHeaderLabels(n_row*[""])
    pb_del.clicked.connect(lambda : del_obj_by_table(w_t, obj))
    return w_t

def del_obj_by_table(table_widget, obj):
...
def add_mandatory_optional_obj(obj, label):
    """
layout to add new attributes for DataConstr or GlobalConstr
    ...
    pb_add.clicked.connect(lambda : add_obj_by_table(table_widget_1, l_cls, obj))
...
def add_obj_by_table(table_widget, l_cls, obj):
    if type(obj) == GlobalConstr:
...
def get_layout_rciftab_obj(obj):
    layout = QtWidgets.QVBoxLayout()
    ...
    button_to_obj.clicked.connect(lambda : add_rcif_to_obj(obj, _text_edit.toPlainText()))
    return layout

def add_rcif_to_obj(obj, text):
    obj_cls = type(obj)
    obj_new = None
