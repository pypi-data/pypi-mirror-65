...
class AppInsightsPIIStrippingFormatter(logging.Formatter):
    ...
    def format(self, record: logging.LogRecord) -> str:
        ...
        if isinstance(exception, AutoMLException):
            ...
            pii_free_msg = exception.pii_free_msg()
            message = exception._exception_msg_format(
                exception.__class__.__name__,
                pii_free_msg,
                exception._inner_exception.__class__.__name__,
                exception._serialize_json(
        ...
        if is_exception_stacktrace_loggable():
            ...
            traceback_obj = getattr(record, 'exception_tb_obj', None)
            if traceback_obj is None:
                traceback_msg = 'Not available (exception was not raised but was returned directly)'
            else:
                traceback_msg = ''.join(_CustomStackSummary.format_whitelisted_paths(
        ...
        properties['exception_traceback'] = traceback_msg
