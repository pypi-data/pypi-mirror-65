...
class Utils:
    ...
    def obfuscate_by_type(cls, data, field_name, obfuscation_type):
        ...
        if obfuscation_type == Obfuscation_Type.JSON:
            ...
            if matches and cls.starts_with(matches.groups()[0], "\"{}\"".format(field_name)):
                separator_reg_exp = '(?i)(\\"{}\\"\\s*:\\s*\\")'.format(re.escape(field_name))
                separator_matches = re.search(separator_reg_exp, data)
                {"username": "Some username", "password": "Super secret that should not be seen"}
                start_index = data.index(matches.groups()[0]) + len(separator_matches.groups()[0])
        ...
        if obfuscation_type == Obfuscation_Type.URL:
            ...
            if matches and cls.starts_with(matches.groups()[0].lstrip("?").lstrip("&"), field_name):
                ...
                separator_reg_exp = "(?i)((\\?|\\&){}=)".format(re.escape(field_name))
                separator_matches = re.search(separator_reg_exp, data)
                stripped_separator_matches = separator_matches.groups()[0].lstrip("?").lstrip("&")
                start_index = data.index(stripped_match) + len(stripped_separator_matches)
                try:
    ...
    def obfuscate_body(cls, body):
        ...
        if "body" in log_label_obfuscate:
            ...
            for field_name, obfuscation_type in log_label_obfuscate["body"].items():
    ...
    def obfuscate_response(cls, response):
        ...
        if "response" in log_label_obfuscate:
            ...
            for field_name, obfuscation_type in log_label_obfuscate["response"].items():
    ...
    def obfuscate_url(cls, url):
        ...
        if "url" in log_label_obfuscate:
            for field_name, obfuscation_type in log_label_obfuscate["url"].items():
