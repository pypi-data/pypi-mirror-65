...
from portfoliotools.screener.utility.util import getHistoricStockPrices, ema, cum_pattern, smooth_avg, slope, backTestStrategy
...
class StockDetail( object ):
    ...
    def get_obv_results( self ):
        ...
        try:
            ...
            info['Passive Portfolio'] = round(details['Passive Portfolio'].values[0],2)
            info['Active Portfolio']  = round(details['Passive Portfolio'].values[0],2)
        except:
            ...
            info['OBV Slope']         = -999
            info['OBV Signal']        = ''
            info['Passive Portfolio'] = -999
            info['Active Portfolio']  = -999
        finally:
            return info
    ...
    def get_rsi_results(self):
        ...
        try:
            ...
            info['Passive Portfolio']   = round(details['Passive Portfolio'][0],2)
            info['Active Portfolio']    = round(details['Active Portfolio'][0],2)
        except:
    ...
    def get_williams_r_results(self):
        ...
        try:
            ...
            try:
                info['%R'], info['%D'], info['WilliamR Signal'], info['Passive Portfolio'], info['Active Portfolio']  = self.get_williams_r_details()[2].tail(1)[['%R', '%D', 'WilliamR Signal', 'Passive Portfolio', 'Active Portfolio']].values[0]
            except:
                info['%R'], info['%D'], info['WilliamR Signal'], info['Passive Portfolio'], info['Active Portfolio']  = -999, -999, "", -999,-999
        except:
            log.error("Error calculating William's R Results - {0}".format(self.name))
        finally:
            info['%R'] = round(info['%R'],2)
            info['%D'] = round(info['%D'],2)
            info['Passive Portfolio'] = round(info['Passive Portfolio'],2)
            info['Active Portfolio'] = round(info['Active Portfolio'],2)
            return info
    ...
    def get_macd_results(self):
        info = dict()
        try:
            info['MACD Line'], info['MACD Indicator'], info['MACD Line to Indicator Diff (Value)'], info['MACD Line to Indicator Diff (Days)'], info['MACD Line to Indicator Diff Rolling Peak'], info['MACD Line to Indicator Diff Rolling Peak per'], info['MACD Signal'], info['Passive Portfolio'], info['Active Portfolio'] = self.get_macd_details().tail(1)[['MACD Line', 'MACD Indicator', 'MACD Line to Indicator Diff (Value)', 'MACD Line to Indicator Diff (Days)', 'MACD Line to Indicator Diff Rolling Peak', 'MACD Line to Indicator Diff Rolling Peak per', 'MACD Signal', 'Passive Portfolio', 'Active Portfolio']].values[0]
        except:
            log.error("Error generating MacD results - {0}".format(self.name))
            info['MACD Line'], info['MACD Indicator'], info['MACD Line to Indicator Diff (Value)'], info['MACD Line to Indicator Diff (Days)'], info['MACD Line to Indicator Diff Rolling Peak'], info['MACD Line to Indicator Diff Rolling Peak per'], info['MACD Signal'] = -1,-1,-1,-1,-1,-1,""
        finally:
