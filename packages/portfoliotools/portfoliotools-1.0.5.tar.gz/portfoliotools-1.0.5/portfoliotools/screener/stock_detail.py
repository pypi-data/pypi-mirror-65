...
class StockDetail( object ):
    ...
    def get_obv_details( self ):
        ...
        try:
            ...
            self._add_to_technical_details(df, 'OBV', cols = ['MA Slope', 'OBV Slope', 'Signal'])
            return df.dropna()
    ...
    def get_rsi_details(self, lookback_period = None):
        ...
        try:
            ...
            self._add_to_technical_details(df, 'RSI', cols = ['RSI', 'RSI Signal'], col_map = {'RSI':'Value','RSI Signal':'Signal'})
            return df.dropna()
    ...
    def get_williams_r_results(self):
        ...
        try:
            ...
            try:
                info['%R'], info['%D'], info['WilliamR Signal']  = self.get_williams_r_details()[2].tail(1)[['%R', '%D', 'WilliamR Signal']].values[0]
            except:
                info['%R'], info['%D'], info['WilliamR Signal']  = -999, -999, ""
        except:
    ...
    def get_williams_r_details(self):
        ...
        self._add_to_technical_details(df, 'Williams R', cols = ['%D', '%R', 'WilliamR Signal'], col_map = {'WilliamR Signal':'Signal'})
    ...
    def get_macd_details(self):
        ...
        try:
            ...
            self._add_to_technical_details(result, 'MACD', cols = ['MACD Signal', 'MACD Line to Indicator Diff (Value)'], col_map = {'MACD Signal':'Signal', 'MACD Line to Indicator Diff (Value)':'Diff Value'})
    ...
    def get_macd_results(self):
        info = dict()
        try:
            info['MACD Line'], info['MACD Indicator'], info['MACD Line to Indicator Diff (Value)'], info['MACD Line to Indicator Diff (Days)'], info['MACD Line to Indicator Diff Rolling Peak'], info['MACD Line to Indicator Diff Rolling Peak per'], info['MACD Signal'] = self.get_macd_details().tail(1)[['MACD Line', 'MACD Indicator', 'MACD Line to Indicator Diff (Value)', 'MACD Line to Indicator Diff (Days)', 'MACD Line to Indicator Diff Rolling Peak', 'MACD Line to Indicator Diff Rolling Peak per', 'MACD Signal']].values[0]
        except:
