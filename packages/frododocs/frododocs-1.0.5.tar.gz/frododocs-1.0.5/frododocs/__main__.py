...
from frododocs import exceptions
from frododocs import config
from frododocs.commands import build, gh_deploy, new, serve
...
class State(object):
    ...
    def __init__(self, log_name='frododocs', level=logging.INFO):
...
config_help = "Provide a specific FrodoDocs config"
dev_addr_help = ("IP address and port to serve documentation locally (default: "
                 "localhost:8000)")
strict_help = ("Enable strict mode. This will cause FrodoDocs to abort the build "
               "on any warnings.")
...
commit_message_help = ("A commit message to use when committing to the "
                       "Github Pages remote branch. Commit {sha} and FrodoDocs {version} are available as expansions")
...
ignore_version_help = "Ignore check that build is not being deployed with an older version of FrodoDocs."
...
def cli():
    """
    FrodoDocs - Project documentation with Markdown.
    """
...
def build_command(clean, config_file, strict, theme, theme_dir, site_dir):
    """Build the FrodoDocs documentation"""
...
def new_command(project_directory):
    """Create a new FrodoDocs project"""
