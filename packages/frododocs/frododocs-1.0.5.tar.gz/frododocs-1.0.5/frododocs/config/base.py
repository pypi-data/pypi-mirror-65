...
log = logging.getLogger('frododocs.config')
...
class Config(utils.UserDict):
    ...
    def load_file(self, config_file):
        try:
            ...
            # FrodoDocs knows and understands ConfigurationErrors
            raise exceptions.ConfigurationError(
                "FrodoDocs encountered as error parsing the configuration file: {}".format(e)
            )
...
def _open_config_file(config_file):
    ...
    if config_file is None:
        config_file = os.path.abspath('frododocs.yml')
...
def load_config(config_file=None, **kwargs):
    ...
    the default `frododocs.yml` filename will loaded.
