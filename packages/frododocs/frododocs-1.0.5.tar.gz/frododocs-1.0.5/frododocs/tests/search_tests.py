...
from frododocs.structure.files import File
from frododocs.structure.pages import Page
from frododocs.structure.toc import get_toc
from frododocs.contrib import search
from frododocs.contrib.search import search_index
from frododocs.config.config_options import ValidationError
from frododocs.tests.base import dedent, get_markdown_toc, load_config
...
class SearchPluginTests(unittest.TestCase):
    ...
    def test_event_on_config_defaults(self):
        plugin = search.SearchPlugin()
        plugin.load_config({})
        result = plugin.on_config(load_config(theme='frododocs', extra_javascript=[]))
    ...
    def test_event_on_config_include_search_page(self):
        plugin = search.SearchPlugin()
        plugin.load_config({})
        config = load_config(theme={'name': 'frododocs', 'include_search_page': True}, extra_javascript=[])
    ...
    def test_event_on_config_search_index_only(self):
        plugin = search.SearchPlugin()
        plugin.load_config({})
        config = load_config(theme={'name': 'frododocs', 'search_index_only': True}, extra_javascript=[])
        result = plugin.on_config(config)
    ...
    @mock.patch('frododocs.utils.write_file', autospec=True)
    @mock.patch('frododocs.utils.copy_file', autospec=True)
    def test_event_on_post_build_defaults(self, mock_copy_file, mock_write_file):
        plugin = search.SearchPlugin()
        plugin.load_config({})
        config = load_config(theme='frododocs')
    ...
    def test_event_on_post_build_search_index_only(self, mock_copy_file, mock_write_file):
        plugin = search.SearchPlugin()
        plugin.load_config({'lang': ['es']})
        config = load_config(theme={'name': 'frododocs', 'search_index_only': True})
        plugin.on_pre_build(config)
        plugin.on_post_build(config)
        self.assertEqual(mock_copy_file.call_count, 0)
        self.assertEqual(mock_write_file.call_count, 1)
