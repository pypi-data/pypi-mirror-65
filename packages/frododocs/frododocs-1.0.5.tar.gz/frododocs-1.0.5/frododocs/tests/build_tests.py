...
from frododocs.structure.pages import Page
from frododocs.structure.files import File, Files
from frododocs.structure.nav import get_navigation
from frododocs.commands import build
from frododocs.tests.base import load_config, LogTestCase, tempdir, PathAssertionMixin
from frododocs.utils import meta
...
class BuildTests(PathAssertionMixin, LogTestCase):
    ...
    @mock.patch('frododocs.utils.write_file')
    @mock.patch('frododocs.commands.build._build_template', return_value='some content')
    ...
    @mock.patch('frododocs.utils.write_file')
    @mock.patch('frododocs.commands.build._build_template', return_value='')
    def test_skip_missing_theme_template(self, mock_build_template, mock_write_file):
        cfg = load_config()
        env = cfg['theme'].get_env()
        with self.assertLogs('frododocs', level='WARN') as cm:
            build._build_theme_template('missing.html', env, mock.Mock(), cfg, mock.Mock())
        self.assertEqual(
            cm.output,
            ["WARNING:frododocs.commands.build:Template skipped: 'missing.html' not found in theme directories."]
        )
    ...
    def test_skip_theme_template_empty_output(self, mock_build_template, mock_write_file):
        ...
        self.assertEqual(
            cm.output,
            ["INFO:frododocs.commands.build:Template skipped: 'main.html' generated empty output."]
        )
        mock_write_file.assert_not_called()
        mock_build_template.assert_called_once()
    ...
    def test_skip_missing_extra_template(self):
        ...
        self.assertEqual(
            cm.output,
            ["WARNING:frododocs.commands.build:Template skipped: 'missing.html' not found in docs_dir."]
        )
    ...
    def test_skip_ioerror_extra_template(self, mock_open):
        ...
        self.assertEqual(
            cm.output,
            ["WARNING:frododocs.commands.build:Error reading template 'foo.html': Error message."]
        )
    ...
    def test_skip_extra_template_empty_output(self):
        ...
        self.assertEqual(
            cm.output,
            ["INFO:frododocs.commands.build:Template skipped: 'foo.html' generated empty output."]
        )
    ...
    def test_populate_page_dirty_modified(self, site_dir):
        ...
        self.assertTrue(page.markdown.startswith('# Bienvenido a FrodoDocs'))
        self.assertTrue(page.content.startswith('<h1 id="welcome-to-frododocs">Bienvenido a FrodoDocs</h1>'))
    ...
    def test_populate_page_read_error(self, docs_dir, mock_open):
        ...
        self.assertEqual(
            cm.output, [
                'ERROR:frododocs.structure.pages:File not found: missing.md',
                "ERROR:frododocs.commands.build:Error reading page 'missing.md': Error message."
            ]
        )
    ...
    #     self.assertEqual(
    #         cm.output,
    #         ["INFO:frododocs.commands.build:Page skipped: 'index.md'. Generated empty output."]
    #     )
    ...
    @tempdir()
    @mock.patch('frododocs.utils.write_file', side_effect=IOError('Error message.'))
    def test_build_page_error(self, site_dir, mock_write_file):
        ...
        self.assertEqual(
            cm.output,
            ["ERROR:frododocs.commands.build:Error building page 'index.md': Error message."]
        )
        mock_write_file.assert_called_once()
