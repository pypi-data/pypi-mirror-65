...
default_message = """Deployed {sha} with FrodoDocs version: {version}"""
...
def _check_version(branch):
    ...
    currentv = parse_version(frododocs.__version__)
    if not previousv:
        log.warn('Version check skipped: No version specificed in previous deployment.')
    elif currentv > previousv:
        log.info(
            'Previous deployment was done with FrodoDocs version {}; '
            'you are deploying with a newer version ({})'.format(previousv, currentv)
        )
    elif currentv < previousv:
        log.error(
            'Deployment terminated: Previous deployment was made with FrodoDocs version {}; '
...
def gh_deploy(config, message=None, force=False, ignore_version=False):
    ...
    message = message.format(version=frododocs.__version__, sha=sha)
