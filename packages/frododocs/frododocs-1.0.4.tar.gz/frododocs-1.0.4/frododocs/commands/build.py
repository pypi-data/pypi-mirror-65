...
from frododocs.structure.files import get_files
...
def get_context(nav, files, config, page=None, base_url=''):
    ...
    return {
        ...
        'frododocs_version': frododocs.__version__,
...
def _build_template(name, template, files, config, nav):
    ...
    if utils.is_error_template(name):
        ...
        # See https://github.com/frododocs/frododocs/issues/77.
        # However, if site_url is not set, assume the docs root and server root
        # are the same. See https://github.com/frododocs/frododocs/issues/1598.
