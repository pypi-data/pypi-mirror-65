...
from frododocs.tests.base import dedent, load_config, LogTestCase
...
class SiteNavigationTests(LogTestCase):
    ...
    def test_nav_external_links(self):
        ...
        with self.assertLogs('frododocs', level='DEBUG') as cm:
            site_navigation = get_navigation(files, cfg)
        self.assertEqual(
            cm.output,
            [
                "DEBUG:frododocs.structure.nav:An absolute path to '/local.html' is included in the "
                "'nav' configuration, which presumably points to an external resource.",
                "DEBUG:frododocs.structure.nav:An external link to 'http://example.com/external.html' "
    ...
    def test_nav_bad_links(self):
        ...
        self.assertEqual(
            cm.output,
            [
                "WARNING:frododocs.structure.nav:A relative path to 'missing.html' is included "
                "in the 'nav' configuration, which is not found in the documentation files",
                "WARNING:frododocs.structure.nav:A relative path to 'example.com' is included "
                "in the 'nav' configuration, which is not found in the documentation files"
            ]
        )
        self.assertEqual(str(site_navigation).strip(), expected)
