...
from frododocs import exceptions, utils
from frododocs.config import base, defaults
from frododocs.config.config_options import BaseConfigOption
...
class ConfigBaseTests(unittest.TestCase):
    ...
    def test_load_from_file(self):
        ...
        config_file = open(os.path.join(temp_dir.name, 'frododocs.yml'), 'w')
        os.mkdir(os.path.join(temp_dir.name, 'docs'))
        try:
            config_file.write("site_name: FrodoDocs Test\n")
            config_file.flush()
            config_file.close()

            cfg = base.load_config(config_file=config_file.name)
            self.assertTrue(isinstance(cfg, base.Config))
            self.assertEqual(cfg['site_name'], 'FrodoDocs Test')
        finally:
    ...
    def test_load_from_open_file(self):
        ...
        config_fname = os.path.join(temp_path, 'frododocs.yml')
    ...
    def test_load_missing_required(self):
        ...
        try:
            config_file.write(
                "site_dir: output\nsite_uri: https://www.maeorg.site\n")
            config_file.flush()
            config_file.close()
    ...
    def test_load_from_file_with_relative_paths(self):
        ...
        config_fname = os.path.join(config_dir.name, 'frododocs.yml')
