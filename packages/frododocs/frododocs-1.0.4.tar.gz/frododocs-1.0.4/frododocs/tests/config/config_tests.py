...
import frododocs
from frododocs import config
from frododocs import utils
from frododocs.config import config_options
from frododocs.exceptions import ConfigurationError
from frododocs.tests.base import dedent
...
class ConfigTests(unittest.TestCase):
    ...
    def test_config_option(self):
        ...
        Allows users to specify a config other than the default `frododocs.yml`.
        """
        expected_result = {
            'site_name': 'Example',
            'pages': [
        ...
        with TemporaryDirectory() as temp_path:
            ...
            config_path = os.path.join(temp_path, 'frododocs.yml')
    ...
    def test_theme(self):
        with TemporaryDirectory() as mytheme, TemporaryDirectory() as custom:
            ...
            results = (
                {
                    'dirs': [os.path.join(theme_dir, 'frododocs'), frododocs_templates_dir],
                    ...
                    'dirs': [os.path.join(theme_dir, 'readthedocs'), frododocs_templates_dir],
                    'static_templates': ['404.html', 'sitemap.xml'],
                    'vars': {
                    ...
                    'dirs': [mytheme, frododocs_templates_dir],
                    'static_templates': ['sitemap.xml'],
                    'vars': {}
                }, {
                    'dirs': [custom, os.path.join(theme_dir, 'readthedocs'), frododocs_templates_dir],
                    'static_templates': ['404.html', 'sitemap.xml'],
                    'vars': {
                        'include_search_page': True,
                        'search_index_only': False,
            ...
            for config_contents, result in zip(configs, results):
                ...
                c = config.Config(schema=(('theme', config_options.Theme(default='frododocs')),))
    ...
    def test_empty_nav(self):
        ...
        conf.load_dict({
            'site_name': 'Example',
            'config_file_path': os.path.join(os.path.abspath('.'), 'frododocs.yml')
        })
        conf.validate()
    ...
    def test_doc_dir_in_site_dir(self):
        ...
        conf = {
            'config_file_path': j(os.path.abspath('..'), 'frododocs.yml')
        }
