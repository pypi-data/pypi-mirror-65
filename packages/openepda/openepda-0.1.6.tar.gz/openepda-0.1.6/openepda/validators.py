...
def _check_building_block(bb_name, bb):
    ...
    if "pin_in" in bb:
        if not bb["pin_in"] in bb["pins"].keys():
            raise ValueError(
                "pin_in '{}' is not present in the list of pins "
                "for in BB {}.".format(bb["pin_in"], bb_name)
            )
    if "pin_out" in bb:
        if not bb["pin_out"] in bb["pins"].keys():
            raise ValueError(
                "pin_out '{}' is not present in the list of pins "
                "for in BB {}.".format(bb["pin_out"], bb_name)
            )
