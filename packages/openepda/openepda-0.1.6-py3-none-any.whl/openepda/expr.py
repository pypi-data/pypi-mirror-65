...
def ensure_num(expr: Union[str, int, float]):
    """Evaluate expression if necessary and return a numeric value.

    Parameters
    ----------
    expr : Union[str, int, float]
        expression to be checked and evaluated if needed.
    ...
    if isinstance(expr, str):
        res = openepda_parser.eval(expr)
        if not isinstance(res, (float, int)):
            raise TypeError(
                "Expression '{}' is evaluated as a non-numeric type {}. "
                "Evaluation result: {}.".format(expr, type(res), res)
            )
    elif isinstance(expr, (float, int)):
        res = expr
    else:
        raise TypeError(
            "Provided expression '{}' cannot be converted to a"
            "numeric values."
        )
    return res
