...
class DashboardView(
    ...
    def get(self, request, all_tasks=False):  # noqa
        ...
        user_tasks_in_progress = models.UserTask.objects.filter(
            status='in progress', user_id=user.pk
        ).all()
        ...
        if len(context['projects']) < max_tasks:
            ...
            for project in projects_available_review:
                if not project.available_review_task(user):
                    continue
        ...
        if len(context['projects']) < max_tasks:
            projects_available_transcription = (
                models.Project.objects.filter(
                    archived=0, tasks__finished_transcription=0
                )
                .distinct()
                .order_by('-priority')
            )
...
class UserTaskUpdateView(
    ...
    def get_context_data(self, **kwargs):
        ...
        project = self.object.task.project
        context['pages'] = project.get_image_urls()
        return context
