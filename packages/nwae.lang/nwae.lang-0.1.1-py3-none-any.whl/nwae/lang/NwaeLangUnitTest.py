...
from nwae.lang.nlp.ut.UtSpellingCorrection import UnitTestSpellingCorrection
...
class NwaeLangUnitTest:
    ...
    def run_unit_tests(self):
        ...
        res_final.update(other_res_obj=res)
        Log.critical('TrieNode (Edit Distance) Unit Test PASSED ' + str(res.count_ok) + ', FAILED ' + str(res.count_fail))

        res = UnitTestSpellingCorrection(ut_params=self.ut_params).run_unit_test()
        res_final.update(other_res_obj=res)
        Log.critical('Spelling Correction Unit Test PASSED ' + str(res.count_ok) + ', FAILED ' + str(res.count_fail))
...
if __name__ == '__main__':
    ...
    ut_params = uthelper.UnitTestParams(
        ...
        dirpath_model        = None,
        dirpath_sample_data  = config.get_config(param=cf.Config.PARAM_NLP_DIR_SAMPLE_DATA,)
    )
