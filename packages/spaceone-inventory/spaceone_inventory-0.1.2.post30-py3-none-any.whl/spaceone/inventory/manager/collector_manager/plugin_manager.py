...
class PluginManager(BaseManager):
    ...
    def get_secrets_from_plugin_info(self, plugin_info, domain_id, secret_id=None):
        ...
        secret_id_list = self._get_secret_id_list(plugin_info)
        if secret_id:
    ...
    def _check_plugin_info(self, plugin_info):
        ...
        for item in mendatory:
            if not item in plugin_info:
                raise ERROR_NO_PLUGIN_PARAMETER(param=item)

        return True

    def _get_secret_id_list(self, plugin_info):
        """
        ...
        # TODO: secret from provider

        _LOGGER.debug(f'[_get_secret_id_list] {secret_id}, {secret_group_id}')
