...
class FilterManager(BaseManager):
        ...
        Example:
            filter_format:
                'filter_format': [
                    ...
                    {'key':'instance_type', 'name':'Instance Type', 'type':'str', 'resource_type': 'CUSTOM'}
                ]
    ...
    def _search_resources(self, query_per_resources, filter_format_by_key, secret_id_list):
        ...
        for res_type, query in query_per_resources.items():
            ...
            # Ask to manager
            try:
                _LOGGER.debug(f'[_search_resources] query: {query}, key={change_key[0]}')
                value_list, filtered_secret_id_list = mgr.query_resources(query, only=[change_key[0]])
