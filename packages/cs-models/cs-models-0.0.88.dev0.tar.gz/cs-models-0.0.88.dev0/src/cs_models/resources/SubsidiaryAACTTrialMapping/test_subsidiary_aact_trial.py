...
from src.cs_models.resources.SubsidiaryAACTTrialMapping import (
    SubsidiaryAACTTrialMapping,
)
...
class SubsidiaryAACTTrailResourceTestCase(TestCase):
    def setUp(self):
        super(SubsidiaryAACTTrailResourceTestCase, self).setUp()
        self.inst = SubsidiaryAACTTrialMapping()
        ...
        self.subsidiary_aact_trial_mapping1 = self.inst.create({
            'nct_id': 'NCT03331640',
            'subsidiary_id': self.subsidiary2['id'],
        })

        self.subsidiary_aact_trial_mapping2 = self.inst.create({
            'nct_id': 'NCT03993847',
    ...
    def test_create_violates_unique_constraint(self):
        self.assertRaises(
            IntegrityError,
            self.inst.create,
            {'nct_id': self.subsidiary_aact_trial_mapping1['nct_id'],
             'subsidiary_id': self.subsidiary_aact_trial_mapping1[
                 'subsidiary_id']
             },
        )
    ...
    @parameterized.expand([
        ('nct_id', 'subsidiary_aact_trial_mapping1', 'nct_id', 1),
        ('subsidiary_id', 'subsidiary_aact_trial_mapping1', 'subsidiary_id',
         1),
    ])
    ...
    def test_one(self, field_name):
        resp = self.inst.one({
            field_name: self.subsidiary_aact_trial_mapping1[field_name],
        })
        second_equals_first(
            self.subsidiary_aact_trial_mapping1,
            resp,
        )
