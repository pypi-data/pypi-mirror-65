...
URL_BASE = 'https://db.ygorganization.com'
VERBOSE = False
def DEBUG(s):
    if VERBOSE:
...
async def MainLoop(queue):
    try:
        ...
        async def ResolveSingleJob(jobs):
            ...
            assert(path[0] == '/')
            async with aiohttp.request(method='GET', url=(URL_BASE+path)) as r:
                r.raise_for_status()
                body = await r.text()
        ...
        while True:
            if needManifest:
                ...
                async with aiohttp.request(method='GET', url=(URL_BASE + ('/manifest/%d' % cacheRevision))) as r:
                    r.raise_for_status()
                    if 0 < cacheRevision:
