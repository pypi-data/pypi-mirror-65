...
CONFIG_PATH = (f'/Users/sashapodoprigora'
               f'/Documents/GitHub/us-oa-scripts/oa_data_package'
               f'/hellofresh_data/configs')
...
class DataReader():
    ...
    def instantiate_config(self):
        ...
        """
        self.config = \
            yaml.safe_load(open(os.path.join(CONFIG_PATH,
                                             'data_master_config.yml')))

    def instantiate_bodega_creds(self):
        """
            Retrieve bodega credentials for DDL
        """
        self.bodega_user_path = self.config['database']['user']
        self.bodega_password_path = self.config['database']['password']
...
class DataWriter(DataReader):
    ...
    def __init__(self,
        ...
        logging.basicConfig()
        self._logger = logging#logging_setup.init_logging('DataWriter')
    ...
    def instantiate_bodega_variables(self):
        ...
        self.default_trigger_name = \
            self.config['database_sql']['default_trigger_name']

        self.source_schema = self.config['database_sql']['source_schema']
