...
from apsara.utils import speaker


def parse_country_details(COUNTRY_DETAILS, config):
    try:
        x = PrettyTable()
        ...
        if 'speech' in config:
            speaker.online_text_to_speach(
                'covid-19-status-'+total_confirmed_cases,
                'Covid 19 India Status. There are total' +
                total_confirmed_cases + 'cases in India.'
            )
            speaker.online_text_to_speach(
                'covid-19-status-'+delta_confirmed_cases,
                'and there have been increse in cases by' +
                delta_confirmed_cases + 'Stay home Stay Secured'
            )
