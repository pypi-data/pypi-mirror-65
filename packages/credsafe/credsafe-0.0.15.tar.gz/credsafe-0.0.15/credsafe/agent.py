...
class Agent(object):
    ...
    def rm(self, id: str, pw: str, k: str) -> bool:
        return Broker(app_name=self.__app_name, username=mac(id, self.__hmac_key)+mac(pw, self.__hmac_key))\
            .rm(mac(k, self.__hmac_key))

    def destroy(self, id: str, pw: str) -> bool:
        return Broker(app_name=self.__app_name, username=mac(id, self.__hmac_key)+mac(pw, self.__hmac_key)).destroy()
