...
class Agent(object):
    ...
    def set(self, id: str, pw: str, k: str, v: Any) -> Any:
        Broker(app_name=self.__app_name, username=mac(f"{id}:{pw}", self.__hmac_key))\
    ...
    def get(self, id: str, pw: str, k: str) -> Any:
        return self.__decrypt(Broker(app_name=self.__app_name, username=mac(f"{id}:{pw}", self.__hmac_key))
