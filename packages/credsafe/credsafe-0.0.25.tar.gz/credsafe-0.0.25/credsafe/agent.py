...
class Agent(object):
    def __init__(self, app_name: str, key_pair: key_pair_format, aes_key: bytes, hmac_key: bytes) -> None:
        self.__hmac = lambda _: self.__check() and mac(_, hmac_key)
        self.__skey = lambda: self.__check() and bytes(randb(len(aes_key))[i] ^ aes_key[i] for i in range(len(aes_key)))
        self.__aese = lambda k, v: self.__check() and AESCipher(k).encrypt(v)
        self.__aesd = lambda k, v: self.__check() and AESCipher(
            EasyRSA(private_key=key_pair["private_key"]).decrypt(k)
        ).decrypt(v)
        self.__rsae = lambda k: self.__check() and EasyRSA(public_key=key_pair["public_key"]).encrypt(k)
    ...
    def __decrypt(self, v: str) -> Any:
        ...
        if hash == self.__hmac(v):
            return jl(self.__aesd(b64d(k), v))
