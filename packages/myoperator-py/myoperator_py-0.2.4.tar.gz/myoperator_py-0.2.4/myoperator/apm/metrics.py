...
def add_app_name_to_metric(metric_name, seperator=METRIC_NAME_SEPERATOR):
    global APP_NAME
...
def create_http_request_latency_histogram_metric(metric_name=BASE_HTTP_LATENCY_METRIC_NAME):
    metric_name = add_app_name_to_metric(metric_name)
    return metric_name
...
def create_http_request_metrics(status_code, duration_ms, exception_type_class=None):
    """
        ...
        duartion_ms float: Time taken for request in milliseconds
         str: This string will be added to the start of each metric generated.

        Returns tuple: (<histogram_metric_name>, <guage_metric_name>, <exception_metric_name>)
                        exception_metric_name is None is status code is other than range (500, 599) and no exception_type_class is provided
    """
...
def create_function_duration_metrics(duration_ms, function_name, host, status, exception_type_class=None):
    """
