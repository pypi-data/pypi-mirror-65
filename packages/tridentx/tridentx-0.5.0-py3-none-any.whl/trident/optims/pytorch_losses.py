...
__all__ = ['MSELoss', 'CrossEntropyLoss', 'NullLoss', 'BCELoss', 'F1ScoreLoss', 'L1Loss', 'SmoothL1Loss', 'L2Loss',
...
class MS_SSIMLoss(_Loss):
    ...
    def forward(self, output, target):
        return 1-msssim(output, target, window_size=self.window_size, size_average=True, normalize=True)
...
class PerceptionLoss(_Loss):
    def __init__(self, vgg16, reduction="mean", reduced=False):
        ...
        for name, module in self.vgg_model.named_modules():
    ...
    def forward(self, output, target):
        ...
        _ = self.vgg_model(output)
        for item in self.layer_name_mapping.values():
            output_features[item] = getattr(self.vgg_model, item).output

        _ = self.vgg_model(target)
        for item in self.layer_name_mapping.values():
            target_features[item] = getattr(self.vgg_model, item).output.detach()

        loss = 0
        for i in range(len(self.layer_name_mapping)):
            b,c,h,w=output_features.value_list[i].shape
            loss += ((output_features.value_list[i] - target_features.value_list[i]) ** 2).mean()/(h*w)
        return loss
