...
__all__ = ['is_nan','is_abnormal_number','reshape','argmax','gram_matrix','log_sum_exp','reduce_max','reduce_min','reduce_mean','reduce_sum','expand_dims','shuffle','random_choice','meshgrid','element_cosine_distance','gram_matrix','get_rotation_matrix2d','warp_affine','binary_crossentropy']
...
def is_abnormal_number(x):
    if isinstance(x,torch.Tensor):
        return torch.isnan(x).any() or torch.isinf(x).any()or torch.isinf(-x).any()
    elif isinstance(x,nn.Module):
        for para in x.parameters():
            if torch.isnan(para).any() or torch.isinf(para).any()or torch.isinf(-para).any():
                return True
        return False
    elif isinstance(x, np.ndarray):
