...
class Iterator(object):
    ...
    @property
    def palette(self):
        if isinstance(self._label, MaskDataset) and self._label.expect_data_type in [ExpectDataType.label_mask,
                                                                                     ExpectDataType.color_mask]:
        ...
        try:
            ...
            if isinstance(self.label, BboxDataset):
                ...
                if hasattr(self.data, 'image_transform'):
                    data = self.data.image_transform(data)
                    # stop = time.time()
                    # print('data image_transform:{0}'.format(stop - start))
                    # start = stop
                if hasattr(self.label, 'image_transform'):
                    label = self.label.image_transform(label)
                    # stop = time.time()
                    # print('label image_transform:{0}'.format(stop - start))
                    # start = stop
            else:
            ...
            if self.signature is None or len(self.signature) == 0:
                ...
                if label is not None:
                    self.signature['label' if self.label.symbol is None or len(
                        self.label.symbol) == 0 else self.label.symbol] = label.shape if isinstance(label,
                                                                                                    np.ndarray) else \
                        type(
                        label)
                if unpair is not None:
                    self.signature['unpair' if self.unpair.symbol is None or len(
                        self.unpair.symbol) == 0 else self.unpair.symbol] = unpair.shape
                # stop = time.time()
                # print('signature:{0}'.format(stop - start))
                # start = stop
