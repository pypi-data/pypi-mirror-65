...
class LearningPlayer(Player):
    ...
    def _derive_move_stack_bridge(self, game, player):
        # pad to achieve timestep length
        feature_stack = [np.concatenate([self.get_hand_vector(game, player, add_hand_sizes=False),
    ...
    def get_hand_vector(self, game, player: TurnPosition, add_hand_sizes=True):
        hand = game.get_hand(player)
        vector = np.zeros(len(Card) + 3)
        for i, card in enumerate(Card):
            vector[i] = hand.count(card)

        if game.is_betting_complete() and add_hand_sizes:
