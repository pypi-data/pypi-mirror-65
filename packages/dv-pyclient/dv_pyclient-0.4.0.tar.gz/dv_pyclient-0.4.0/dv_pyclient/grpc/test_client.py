...
def dataSourceQuery(stub):
    ...
    l1_filters = [
        {"col": "trans", "type": "String", "filter": ["BUY"]},
        {"col": "symbol", "type": "String", "filter": ["RHAT"]},
        {"col": "currency", "type": "String", "filter": ["USD"]},
    ]
    pc = list(map(lambda c: api.ProjectColumn(name=c[0], type=c[1]), zip(project_cols, column_types)))
    l1fs = list(map(lambda c: api.QueryFilter(stringFilter=api.StringFilter(name=c['col'], type=c['type'], stringFilter=c['filter'])) , l1_filters))
