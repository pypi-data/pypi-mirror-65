...
class Mailbox(models.Model):
    ...
    owner = models.ForeignKey(User, verbose_name=_('Owner'), on_delete=models.CASCADE,
                              blank=True, null=True, related_name='mailboxes')
    ...
    def _get_dehydrated_message(self, msg, record):
        ...
        if msg.is_multipart():
            ...
            file_content = ContentFile(BytesIO(msg.get_payload(decode=True)).getvalue())

            attachment.document.save(file_name, file_content)
    ...
    def _process_save_original_message(self, message, msg):
        ...
        if settings['compress_original_message']:
            with NamedTemporaryFile(dir=django_settings.MEDIA_ROOT, delete=True, suffix=".eml.gz") as fp_tmp:
                with gzip.GzipFile(fileobj=fp_tmp, mode="w") as fp:
                    fp.write(message.as_string().encode('utf-8'))
                msg.eml.save(fp_tmp.name, File(fp_tmp), save=False)
        else:
            msg.eml.save('message.eml', ContentFile(message.as_string()), save=False)
...
class IncomingMessageManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(outgoing=False)


class OutgoingMessageManager(models.Manager):
    def get_queryset(self):
        return super().get_queryset().filter(outgoing=True)
...
class Message(models.Model):
    mailbox = models.ForeignKey(Mailbox, related_name='messages', verbose_name=_('Mailbox'), on_delete=models.CASCADE)
    subject = models.CharField(_('Subject'), max_length=255)
    message_id = models.CharField(_('Message ID'), max_length=255)
    in_reply_to = models.ForeignKey('django_mailbox.Message', related_name='replies',
                                    blank=True, null=True, verbose_name=_('In reply to'), on_delete=models.CASCADE)
    from_header = models.CharField(_('From header'), max_length=255)
    to_header = models.TextField(_('To header'))
    outgoing = models.BooleanField(_('Outgoing'), default=False, blank=True)
    body = models.TextField(_('Body'))
    encoded = models.BooleanField(_('Encoded'), default=False, help_text=_('True if the e-mail body is Base64 encoded'))
    processed = models.DateTimeField(_('Processed'), auto_now_add=True)
    read = models.DateTimeField(_('Read'), default=None, blank=True, null=True)
    eml = models.FileField(_('Raw message contents'), null=True, upload_to=upload_to,
    ...
    def text(self):
        ...
        """
        return utils.get_body_from_message(self.get_email_object(), 'text', 'plain').replace('=\n', '').strip()
    ...
    def html(self):
        ...
        """
        return utils.get_body_from_message(self.get_email_object(), 'text', 'html').replace('\n', '').strip()
    ...
    def _rehydrate(self, msg):
        ...
        if msg.is_multipart():
            ...
            try:
                attachment = MessageAttachment.objects.get(pk=msg[settings['attachment_interpolation_header']])
...
class MessageAttachment(models.Model):
    message = models.ForeignKey(Message, related_name='attachments', null=True, blank=True,
                                verbose_name=_('Message'), on_delete=models.CASCADE)
    headers = models.TextField(_('Headers'), null=True, blank=True)
    document = models.FileField(_('Document'), upload_to=upload_to)
