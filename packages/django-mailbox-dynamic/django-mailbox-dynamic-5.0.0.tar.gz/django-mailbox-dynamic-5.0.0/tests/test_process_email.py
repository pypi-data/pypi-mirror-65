...
from tests.base import EmailMessageTestCase
...
class TestProcessEmail(EmailMessageTestCase):
    def test_message_without_attachments(self):
        ...
        self.assertEqual(
            msg.from_header,
            'User Random <user@random.test>',
        )
        self.assertEqual(
            msg.to_header,
            'Daniel Pantoja <panttojo@yandex.com>',
        )
    ...
    def test_message_saved(self):
        ...
        assert msg.eml is not None
        assert msg.eml.name.endswith('.eml') is True

        saved_file = os.path.join(settings.MEDIA_ROOT, msg.eml.name)
    ...
    def test_message_compressed(self):
        ...
        compressed_file = os.path.join(settings.MEDIA_ROOT, msg.eml.name)

        assert compressed_file.endswith('gz') is True

        f = gzip.open(compressed_file, 'rb')
        file_content = f.read()
        assert file_content == self._get_email_as_text('generic_message.eml')
