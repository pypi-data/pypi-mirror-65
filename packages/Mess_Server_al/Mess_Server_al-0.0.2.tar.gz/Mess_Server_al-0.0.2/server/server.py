...
from serverapp.config_window import ConfigWindow
...
from config.settings import MAX_CONNECTION, TIMEOUT, \
    ...
    SERVER_DATABASE, DEFAULT_HOST, DEFAULT_PORT
...
def main():
    ...
    if not os.path.exists('server.ini'):
        with open('server.ini', 'w', encoding='utf-8') as f:
            ...
            f.write(f'database_path = {os.getcwd()}\n')
            f.write(f'database_file = {SERVER_DATABASE}\n')
            f.write(f'default_port = {DEFAULT_PORT}\n')
            f.write(f'listen_address = {DEFAULT_HOST}\n')
