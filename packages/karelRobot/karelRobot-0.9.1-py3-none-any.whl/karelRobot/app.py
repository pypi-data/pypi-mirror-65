import tkinter
import threading
from karelRobot.settings import SettingsDialog
from karelRobot.karel import Orientation, Point, Wall, Cell, World, Karel, KarelError
import tkinter.filedialog
import tkinter.messagebox
import configparser
import karelRobot.superkarel
import sys
import os
...
class KarelUI:
    ...
    def SetKarel(self,karel):
        ...
        karelRobot.superkarel.g_Karel = self.m_Karel
        self.ReDraw()

    def NewKarel(self):
        world = World(5,8)
        karel = Karel(world, Point(1, 1), Orientation.EAST)
        self.SetKarel(karel)
        self.SetFile('')
    ...
    def ConfigWorld(self):
        ...
        if settings.result=="ID_OK":
            ...
            karel = Karel(world, Point(settings.m_Data['x'], settings.m_Data['y']), Orientation.EAST,settings.m_Data['beepers'])
            self.SetKarel(karel)
...
class Application(tkinter.Frame):
    ...
    def createWidgets(self):
        ...
        buttons['run'] = tkinter.Button(self, text='执行',image=self.iconStart,command=lambda: self.karelUI.Exec(self.callback))
