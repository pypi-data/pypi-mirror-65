...
class EpochEvaluator(FixedSplitEvaluator):
    ...
    def evaluate(self, model, data):
        ...
        assert self.scoring_step_size <= model.epochs

        trained_epochs = 0
        result = {}
        while (trained_epochs < model.epochs):
            if (model.epochs - trained_epochs) < self.scoring_step_size:
                epochs = model.epochs - trained_epochs
            else:
...
class CvEpochEvaluator(Evaluator):
    ...
    def evaluate(self, model, data):
        ...
        result_lists = {}
        for epoch, _ in results[0].items():
            result_lists[epoch] = defaultdict(list)

        for split in range(self.cv.get_n_splits()):
            for epoch, scoring in results[split].items():
                for score, value in scoring.items():
                    result_lists[epoch][score].append(value)

        results['mean'] = {}
        results['stdev'] = {}
        for epoch, scoring in result_lists.items():
            results['mean'][epoch] = {}
            results['stdev'][epoch] = {}
            for metric, values in scoring.items():
                results['mean'][epoch][metric] = statistics.mean(values)
                results['stdev'][epoch][metric] = statistics.pstdev(values)
