...
class SentimentAnalysisSpanish:
    def __init__(self):
        ...
        with open(Path(__file__).parent / 'tokenizer.pickle', 'rb') as handle:
            self.tokenizer = pickle.load(handle)

        # load json and create model
        json_model_keras = open(Path(__file__).parent / 'model.json', 'r')
        loaded_model_json = json_model_keras.read()
        json_model_keras.close()
        self.loaded_model = model_from_json(loaded_model_json)

        # load weights into new model
        self.loaded_model.load_weights(Path(__file__).parent / "model.h5")
