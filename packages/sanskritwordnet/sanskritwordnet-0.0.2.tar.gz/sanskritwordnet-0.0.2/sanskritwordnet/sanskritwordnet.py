...
class Lemmas:
    def __init__(
            self, session, host, lemma=None, transliteration=None, pos=None, morpho=None, uri=None, dcs=None, token=None
    ):
        self.session = session
        self.host = host
    ...
    def get(self):
        if self.json is None:
            ...
            elif self.dcs is not None:
                self.json = self.session.get(
                    f"{self.host}/api/dcs/{self.dcs}?format=json"
                ).json()["results"]
            else:
                if not self.lemma:
...
class SanskritWordNet:
    def __init__(self, host="http://161.35.46.92", token=None):
    ...
    @lru_cache(maxsize=None)
    def lemmas_by_dcs(self, dcs):
        return Lemmas(self.session, self.host, dcs=dcs)
