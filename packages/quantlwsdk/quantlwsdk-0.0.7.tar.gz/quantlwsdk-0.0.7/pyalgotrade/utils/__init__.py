...
class cusHoldingPostion():
    ...
    def ontrade(self,cusTrade):
        ...
        symbol_ = cusTrade.symbol
        side_ = cusTrade.side
        vol_ = cusTrade.volume
        price_ = cusTrade.price
        time_ = cusTrade.created_at
        # 如果是平仓指令，则需要减去持仓
        if cusTrade.positionEffect in [PositionEffect_Close, PositionEffect_CloseToday,
                                      PositionEffect_CloseYesterday]:

            assert self.symbol==symbol_
            if side_==1:
                assert self.positionSide==2
            if side_==2:
                assert self.positionSide==1


            assert self.volume>=vol_

            self.volume=self.volume-vol_

        if cusTrade.positionEffect in [PositionEffect_Open]:

            assert self.symbol == symbol_
            assert self.positionSide == side_



            self.vwap = (self.vwap * self.volume+ price_ * vol_) / (self.volume + vol_)
            self.volume=self.volume+vol_
