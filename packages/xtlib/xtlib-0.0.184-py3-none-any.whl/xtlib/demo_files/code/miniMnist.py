...
class Trainer():
    ...
    def log_stats(self, epoch, steps, data_len, loss, total_correct, total, model, device, 
        ...
        if self.train_writer2:
            self.train_writer2.add_scalar('Loss', train_loss, epoch)
            self.train_writer2.add_scalar('Acc', train_acc, epoch)
            self.train_writer2.flush()

        if self.test_writer:
            self.test_writer.add_scalar('Loss', test_loss, epoch)
            self.test_writer.add_scalar('Acc', test_acc, epoch)
            self.test_writer.flush()

        if self.test_writer2:
            self.test_writer2.add_scalar('Loss', test_loss, epoch)
            self.test_writer2.add_scalar('Acc', test_acc, epoch)
    ...
    def init_xt_run(self, logging, args):
        # init xtlib
        self.run = None
        ...
        if args.xtlib and os.getenv("XT_RUN_NAME"):
            ...
            self.run = XTRun(xt_logging=logging, aml_logging=logging, checkpoints_enabled=logging)
    ...
    def init_tensorboard(self, logging, local_output_dir, args):
        # init tensorboard
    ...
    def init_stuff(self):
        ...
        self.init_xt_run(logging, args)
        self.init_tensorboard(logging, local_output_dir, args)
