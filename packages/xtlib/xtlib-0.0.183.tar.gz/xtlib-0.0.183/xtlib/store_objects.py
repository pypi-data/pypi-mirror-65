...
class StoreBlobObjs():
    ...
    def _list_directories(self, container, path, wc_target, subdirs=0):
        ...
        dd = {"store_name": "XT Store ({})".format(self.provider_name)}
        #console.print("dd=", dd)
    ...
    def create_next_run_core(self, ws_name, is_parent):
        ...
        '''
        #console.print("create_next_run_core: ws_name=", ws_name)
        self._check_ws_name(ws_name)    
        ...
        next_run = int(text)
        flag_created = False
        #console.print("next_run=", next_run)
        ...
        for i in range(10):
            ...
            try:
                if i:
                    console.print("trying to create flag file: ", fn_flag)
            ...
            next_run += 1
            #console.print("created flag file=", fn_flag)

        if not flag_created:
            errors.service_error("create_next_run_core timed out trying to find next run directory")

        # TODO: there is a timing issue that rfernand2 has experienced on 6/3/2019 - if xt gets control-c interuped 
        ...
        #console.print("updated next_run: fn=", fn_next)

        if is_parent:
            # create NEXT_RUN_NAME (for extra safety, ensure file doesn't already exist)
            blob_fn = run_dir + "/" + constants.WORKSPACE_NEXT
            self._create_blob(ws_name, blob_fn, "1", True)
    ...
    def create_next_child_core(self, ws_name, parent_name):
        ...
        # get next run number
        fn_next = self._run_path(parent_name)  + "/" + constants.WORKSPACE_NEXT
        #console.print("reading next num, fn_next=", fn_next)
    ...
    def create_info_container_if_needed(self):
        ...
        if not self.provider.does_container_exist(info_container):

            # MULTIPROCESS DANGER: here is the first part that can fail if someone else "got there first"
            if self.provider.create_container(info_container):
                ...
                # create the JOBS dir
                jobs_dir_holder_fn = constants.JOBS_DIR + "/" + constants.HOLDER_FILE
    ...
    def read_job_info_file(self, job_name):
        fn_job = self._get_job_path_fn(job_name, constants.JOB_INFO_FN)
        return self._read_blob(constants.INFO_CONTAINER, fn_job)
    ...
    def get_job_names(self):
        ...
        names = [ b.name[5:-1] for b in self.provider.list_blobs(constants.INFO_CONTAINER, path=constants.JOBS_DIR + "/job", recursive=False) ]
        #console.print("names=", names)
...
class BlobsObject():
    ...
    def list_directories(self, path, subdirs=0):
        ...
        # if path:
        #     if not path.startswith("/"):
        #         path = self.container + "/" + self._expand_path(path, True)
        # else:
        #     path = self.container + "/" + self._expand_path("", True)

        # ensure blob directory exists
        #console.print("checking for exists: directory=", directory, ", path=", path)

        # FIX FIX FIX 
        # if not self.store.provider.does_blob_exist(container, path):
        #     errors.user_error("blob directory doesn't exist: {}/{}".format(container, path))
