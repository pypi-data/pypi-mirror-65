...
class PlotBuilder():
    ...
    def build_data_frames(self):
        ...
        for record in self.run_log_records:
            ...
            if not "x" in explicit:
                self.x_col = self.get_actual_x_column(metric_sets, self.x_col)
    ...
    def get_actual_x_column(self, metric_sets, default_x_col):
        x_col = None
        ...
        for ms in metric_sets:
            ...
            if "$step_name" in keys:
                records = ms["records"]
                x_col = records[0]["$step_name"]
            elif default_x_col:
                x_col = default_x_col
            else:
                # try guessing from common names (and _index_, sometimes added by XT)
                x_names = ["epoch", "step", "iter", "epochs", "steps", "iters", "_index_"]
                for xn in x_names:
                    if xn in keys:
                        x_col = xn
                    break
