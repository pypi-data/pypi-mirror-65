...
def create_parser():
    ...
    parser.add_argument('-n', '--no_data_value', type=int, default=0,
                        help='Value that describes pixels with no data in the classes_data file (default: 0).')
...
def run_network(args):
    ...
    """
    image_paths = [os.path.join(args.images_folder, image_name) for image_name in args.image_names]
    classified_path = os.path.join(args.images_folder, args.classes_data_name)
    output_file = args.output if args.output else os.path.join(args.images_folder, "output_classified.tif")
