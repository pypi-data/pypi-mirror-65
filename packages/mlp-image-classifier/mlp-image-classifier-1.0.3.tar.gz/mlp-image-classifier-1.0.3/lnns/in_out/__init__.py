...
def plot_to_file(x, y, path, title="Neural Network MLPClassifier: Network error decline", log_function=print):
    plt.plot(x, y)
    plt.ylabel('cost')
...
def run_algorithm_images(image_paths, classified_path, no_data_value, hidden_layer_size, activation, iterations,
    ...
    try:
        ...
        if feedback:
            if plot_path:
                plot_to_file(range(len(net.network.loss_curve_)), net.network.loss_curve_, plot_path, log_function)
        else:
