...
#int numspots, int numlinked, int numpatterns, SpotToExtract* result)
    ...
    def CreateEstimator(self, psf:Estimator, mod, simfluxFit=True):
        ...
        inst = self._SIMFLUX_CreateEstimator(psf.inst, mod,len(mod), simfluxFit, self.ctx.inst) 
        return Estimator(self.ctx, inst)

    def CreateEstimator_Gauss2D(self, sigma, mod_or_num_patterns, roisize, 
                                    numframes, simfluxEstim=False, defineStartEnd=False, scmos_calib=None) -> Estimator:
