...
def change_timestamp_value(timestamp_keys=[]):
    def wrapper(func):
        @functools.wraps(func)
        def wrapped_func(cls, params):
            change_params = {}
            ...
            for key, value in params.items():
                if key in timestamp_keys:
                    value = _convert_datetime_from_timestamp(value, key)
...
def change_timestamp_filter(filter_keys=[]):
    def wrapper(func):
        @functools.wraps(func)
        def wrapped_func(cls, params):
            query = params.get('query', {})
            ...
            if query_filter:
                query['filter'] = _change_timestamp_condition(query_filter, filter_keys, 'filter')

            if query_filter_or:
                query['filter_or'] = _change_timestamp_condition(query_filter, filter_keys, 'filter_or')
...
def _change_timestamp_condition(query_filter, filter_keys, filter_type):
    change_filter = []
    ...
    for condition in query_filter:
        ...
        if key in filter_keys:
            value = _convert_datetime_from_timestamp(value, f'query.{filter_type}.{key}')
...
def _convert_datetime_from_timestamp(timestamp: dict, key):
    try:
        ...
        raise ERROR_INVALID_PARAMETER_TYPE(key=key, type='Timestamp: {seconds: <second>, nanos: <nano second>}')
