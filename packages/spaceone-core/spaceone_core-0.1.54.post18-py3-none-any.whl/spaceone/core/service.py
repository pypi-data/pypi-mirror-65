...
__all__ = ['BaseService', 'check_required', 'append_query_filter', 'change_datetime_value',
...
def change_datetime_value(filter_keys=[]):
    def wrapper(func):
        @functools.wraps(func)
        def wrapped_func(cls, params):
            query = params.get('query', {})
            ...
            if query_filter:
                query['filter'] = _change_datetime_condition(query_filter, filter_keys, 'filter')

            if query_filter_or:
                query['filter_or'] = _change_datetime_condition(query_filter, filter_keys, 'filter_or')
...
def _change_datetime_condition(query_filter, filter_keys, filter_type):
    change_filter = []
    ...
    for condition in query_filter:
        ...
        if key in filter_keys:
            try:
                ...
                value = datetime.utcfromtimestamp((int(seconds)))
            except Exception as e:
                raise ERROR_INVALID_PARAMETER(key=f'query.{filter_type}.{key}',
                                              reason='The value format is {seconds: <second>, nanos: <nano second>}.')
