...
def get_args():
    args = {
        ...
        "sns_sla_notifier": os.getenv("SLARUNNER_SNS_NOTIFIER_TOPICNAME") or None,
        ...
        "slaLevel": os.getenv("SLA_LEVEL") or 99.99,
...
def run_loop(args):
    ...
    if not args["dry_run"]:
        ...
        if args["sns_sla_notifier"]:
            arn = get_topic_arn(args["sns_sla_notifier"])
            if arn:
                msg = {
                    "level": args["slaLevel"],
                ...
                response = client.publish(
                    TopicArn=arn,
                    MessageStructure='json',
                    Message=json.dumps({'default': msg_encoded})
                )
