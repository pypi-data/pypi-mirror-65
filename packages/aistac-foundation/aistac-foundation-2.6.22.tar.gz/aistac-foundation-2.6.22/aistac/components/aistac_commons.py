...
class AnalyticsCommons(object):
    ...
    class Intent(object):
        ...
        @property
        def top(self):
            return self._intent.get('upper', 1.0)
    ...
    def __init__(self, analysis: dict, label: str = None):
        ...
        if 'associate' not in self._analysis.keys() or 'analysis' not in self._analysis.keys():
            raise ValueError("The analysis does not contain 'associate' or 'analysis' in the first or second level")
