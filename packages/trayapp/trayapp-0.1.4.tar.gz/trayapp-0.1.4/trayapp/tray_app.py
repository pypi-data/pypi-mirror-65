...
class TrayApp:
    ...
    def set_icon(self, icon_or_path: Union[str, Path, type(Image)], icon_size: Tuple[int, int]):
        if isinstance(icon_or_path, (str, Path,)):
            if not (icon_or_path := Path(icon_or_path)).exists():
                raise FileNotFoundError(f'Could not find image: "{icon_or_path}"')
            icon_or_path = Image.open(icon_or_path)
        icon_or_path.thumbnail(icon_size)
        self.app.icon = icon_or_path
