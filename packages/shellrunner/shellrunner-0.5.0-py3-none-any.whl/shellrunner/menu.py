...
class Menu(object):
    ...
    def __select_interactive(self, name, menu):
        ...
        try:
            ...
            return self.__select_by_match(name, menu, selection)

    def __select_by_match(self, name, menu, match):
        if not match or match == self.HERE:
            return self.HERE
    ...
    def __select_from(self, parent, current, indexes, matches):
        ...
        if indexes:
            ...
            index = self.__select_by_match(name, menu, matches.pop())
        ...
        if isinstance(child_value, list):
            return self.__select_from(current, child, indexes, matches)
        else:
            return current, child

    def __select(self, indexes, matches):
        if indexes and matches:
            raise ValueError("Cannot select using both indexes and matches")

        # Indexes and Maches are appended, so we have to reverse the arguments
        ...
        return self.__select_from(parent, current, indexes, matches)

    def select_command(self, indexes=[], matches=[]):
        _, command = self.__select(indexes, matches)
    ...
    def __select_sub_menu(self, indexes=[], matches=[]):
        parent, sub_menu = self.__select(indexes, matches)
    ...
    def __add(self, name, value, indexes=[], matches=[]):
        _, sub_menu = self.__select_sub_menu(indexes, matches)
        sub_menu_name, sub_menu_value = get_only_pair(sub_menu)
    ...
    def add_sub_menu(self, name, indexes=[], matches=[]):
        self.__add(name, [], indexes, matches)

    def add_command(self, name, command, indexes=[], matches=[]):
        self.__add(name, command, indexes, matches)

    def __edit(self, name, value, indexes, matches):
        parent, entry = self.__select(indexes, matches)
        ...
        entry_index = self.__select_by_match(parent_name, parent_value, entry_name)
    ...
    def edit(self, name=None, value=None, indexes=[], matches=[]):
        if not name and not value:
            raise ValueError("Empty edit operation aborted")

        self.__edit(name, value, indexes, matches)

    def __remove(self, indexes, matches, force):
        parent, entry = self.__select(indexes, matches)
        entry_name, entry_value = get_only_pair(entry)
    ...
    def remove(self, indexes=[], matches=[], force=False):
        self.__remove(indexes, matches, force)
