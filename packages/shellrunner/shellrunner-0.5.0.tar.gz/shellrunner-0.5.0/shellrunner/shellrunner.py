...
def run(args):
    ...
    _, command = menu.select_command(args.indexes, args.matches)
    Runner.execute(command)

def add_sub_menu(args):
    conf = Config()
    menu = Menu(conf.commands)
    menu.add_sub_menu(args.name, args.indexes, args.matches)
    conf.set_commands(menu.raw).save()

def add_command(args):
    conf = Config()
    menu = Menu(conf.commands)
    menu.add_command(args.name, args.command, args.indexes, args.matches)
    conf.set_commands(menu.raw).save()

def remove(args):
    conf = Config()
    menu = Menu(conf.commands)
    menu.remove(args.indexes, args.matches, force=args.force)
    conf.set_commands(menu.raw).save()

def edit(args):
    conf = Config()
    menu = Menu(conf.commands)
    menu.edit(args.name, args.value, args.indexes, args.matches)
    conf.set_commands(menu.raw).save()
...
def add_edit_menu(subparsers):
    ...
    edit_parser.add_argument("-v", "--value", required=False,
            default=None, type=str)
