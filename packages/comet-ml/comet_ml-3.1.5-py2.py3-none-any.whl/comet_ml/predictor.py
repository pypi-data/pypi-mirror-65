...
class Predictor(object):
    ...
    def __init__(
        ...
        **defaults
        ...
        self.status_url = url_join(self.base_url, "/isAlive/ping")
        self.predict_url = url_join(self.base_url, "/lc_predictor/predict/")
        self._low_level_http_client = LowLevelHTTPClient(self.base_url)

        try:
            status = self.status()
        except Exception:
            LOGGER.error("Failed to contact Predictor at %s; aborting" % self.base_url)
            raise
