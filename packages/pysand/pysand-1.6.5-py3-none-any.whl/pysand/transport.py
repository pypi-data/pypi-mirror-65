...
def wicks(D, rho_m, mu_m, d_p, angle, rho_p=2650, D_eq=-4196):
    """
    Moye Wicks, Transport of solids at low concentration in horizontal pipes, 196x
    References to paper calculations in parenthesis
    :param D: Pipe diameter [m]
    :param rho_m: Mixture density [kg/m3]
    :param mu_m: Mixture viscosity [kg/ms]
    :param d_p: Particle diameter [mm]
    :param angle: Inclination from vertical [deg]
    :param rho_p: Particle density [kg/m3], default = 2650 (quartz sand)
    :param D_eq: Equivalent pipe diameter in sand filled pipe [m], default = same as pipe diameter
    :return: Critical velocity for particle transport [m/s]
    """

    s = 1.44  # shape factor
    a_rad = np.deg2rad(90-angle)  # Inclination from horizontal in radians

    if D_eq == -4196:
        D_eq = D


    S = (d_p / D) ** (2 / 3) * (D_eq * V_s * rho_m / mu_m)  # (33)

    psi = rho_m**3 * d_p * V_s**4 / ((rho_p - rho_m) * g * mu_m ** 2)  # (part 32)
    ...
    A1 = (rho_p - rho_m) * g * mu_m**2 * (-s*np.sin(a_rad)+np.cos(a_rad)) # undersiden av deler av #32 korrigert for vinkel
    A2 = rho_m**3 * d_p  # oversiden av deler av #32
    V_s = (psi * A1 / A2) ** (1 / 4)  # variant av 32

    S = (d_p / D) ** (2 / 3) * (D_eq * V_s * rho_m / mu_m)
