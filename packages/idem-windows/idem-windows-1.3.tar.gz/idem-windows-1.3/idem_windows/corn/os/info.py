...
async def load_osinfo(hub):
    ...
    hub.corn.CORN.osversion = await hub.corn.init.clean_value(
        "osversion", osinfo.Version
    )

    hub.corn.CORN.osrelease = _windows_os_release_grain(
        caption=osinfo.Caption, product_type=osinfo.ProductType
    )

    hub.corn.CORN.osrelease_info = (int(x) for x in hub.corn.CORN.osversion.split("."))
