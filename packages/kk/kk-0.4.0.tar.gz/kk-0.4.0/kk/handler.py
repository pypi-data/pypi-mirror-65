...
class DiskHandler(tornado.web.StaticFileHandler, BaseHandler):
        ...
        for key, files in self.app.cache.items():
            for doc in files[1]:
                if doc.path.name.lower().find(q) >= 0:
                    self.logger.info(f'search result: {q}, {doc.path}')
    ...
    async def get(self, name, include_body=True):
        ...
        if self.args.q:
            ...
            elif path.suffix.lower() in ['.tar', '.gz', '.bz2'] and tarfile.is_tarfile(path):
                self.preview_tar(path)
            elif path.suffix.lower() in ['.docx', '.doc', '.xls', '.xlsx', '.ppt', '.pptx']:
                self.preview_office()
    ...
    async def post(self, name):
        ...
        if self.args.action and not path.exists():
            ...
            if not self.current_user.admin:
                ...
                self.finish({'err': 1, 'msg': '目标文件已存在'})
            else:
                filename.parent.mkdir(parents=True, exist_ok=True)
                os.symlink(path, filename)
                self.finish({'err': 0, 'msg': '已分享至公共空间'})
            ...
            elif not re.match(r'\.(pdf|txt|mobi|azw|doc|docx|html|htm|rtf|jpeg|jpg|png|gif|bmp)', path.suffix.lower()):
