...
@contextmanager
def operation(app_name, master="yarn-cluster"):
    """
    数据处理封装
    :return:
    """
    ...
    file_save_dict = dict(
            ...
            csv=lambda data_result, output_path: data_result.write.csv(path=output_path, mode="overwrite", header=True),
    ...
    def destruction() -> HandleDataFrameSet:
        ...
        for input_struct in input_structs:
            ...
            file_input_mapping: dict = input_struct.mapping_2_json()
            del file_input_mapping['connectId']
