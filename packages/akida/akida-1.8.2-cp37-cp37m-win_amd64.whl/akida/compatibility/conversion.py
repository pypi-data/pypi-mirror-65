...
def _cnp_max_pooling_split(model, layer):
    ...
    pool_params, pool_weights = _get_weights_params_identity(layer)
    ...
    pool_layer.set_variable("weights", pool_weights)
...
def convert(model):
    ...
    Returns:
        ...
        it is HW compatible.
    """
    new_model = akida.Model(backend=model.get_backend_type())
    nb_layers = model.get_layer_count()
    for i in range(nb_layers):
        layer = model.get_layer(i)
        # If CNP has max pooling, it must be split into 2 CNPs:
        # - one performing the convolution
        # - the other one performing the pooling
        if _cnp_max_pooling(layer):
